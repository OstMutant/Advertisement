...
1. **Plan, once.** If $ARGUMENTS references a plan already fully hashed out earlier in this
   ...
   it end-to-end without checking in again until it's done." **Then stop and wait.** This is the
   one and only gate — do not skip it even if the plan feels obvious.
...
5. **One final report, comprehensive.** When the whole chain above is done (or genuinely blocked
   ...
   to be interrupted until there's something finished to look at.
