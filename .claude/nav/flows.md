...
|---|---|---|
| New bug/improvement/feature request, not yet tracked | `/feature <title>` | Scaffolds `backlog/tasks/<prefix>-NNN-<slug>.md` from the standard template and ranks it in `BACKLOG.md` in the same operation — the only way a new task is filed without leaving it unranked. |
...
| A scoped, already-agreed multi-step task that just needs building | `/autopilot <task>` | One plan, one approval, then implementation/tests/docs/task-lifecycle chained through without further check-ins — the standing per-step Approval Rule still applies to any other task in the same session. |
...
| Watching a long backgrounded script's progress without paying token cost proportional to its raw log volume | `/activity-monitor <script and args>` | Wraps the script, renders a live step checklist from its structured `agentic-output.sh` markers (mechanical, free) or a cheap-model fallback for anything unrecognized — `Monitor` then watches the compact tree file, not raw stdout; see `scripts/activity-monitor/README.md`. |
