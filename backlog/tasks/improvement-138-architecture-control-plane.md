...
explorer. Track A (visual control from already-structured data) is done, its execution history
archived separately. This file's live scope is now Track B (ArchUnit-based contract/test model +
an AI-token-savings hypothesis) plus the absorbed `improvement-135` items 3/5 (the same
"does-a-generated-nav-layer-actually-save-tokens" question, applied to the existing hand-authored
`.claude/nav/` layer).
...
**When:** **Track A** — done (see `completed/tasks/improvement-138-architecture-control-plane-track-a.md`).
**Track B** — blocked on the absorbed item 5 governing rule below: does not start until item 3's
real accumulated data shows a gap in the existing layer, or the user explicitly decides Track B
itself is that evidenced-gap exception.

## Planned — absorb `improvement-135` items 3 and 5, close `improvement-135` (2026-08-28)

User observation: `improvement-135` item 3 ("does `context-loading.md` empirically reduce reads")
and this issue's Track B/B2 ("does a new L0-L5 layer save tokens") are the same open hypothesis —
existing hand-authored nav layer vs. a new generated one — and item 5's governing rule already
directly gates Track B (Finding 3 below). Keeping them in two separate open issues means tracking
one live question twice. Plan:

1. Move `improvement-135`'s item 3 (mechanism built, empirical answer pending real accumulated
   `## Operational notes` data) and item 5 (governing rule: no new `.claude/nav/*`-shaped content
   until items 2-4 show the existing layer earns its cost) into this file verbatim, as a new
   `## Absorbed from improvement-135 (items 3 and 5)` section, right after this plan block.
2. `improvement-135` keeps items 1/2/4 (fully done) and the "2,3,4 — consolidated" preface
   (needed context for items 2/4, which stay there) untouched. Items 3 and 5 are replaced with a
   one-line pointer to this file. `improvement-135`'s file then moves to `backlog/completed/
   tasks/` — fully closed, since everything remaining in it is done.
3. Track A's already-archived execution history
   (`completed/tasks/improvement-138-architecture-control-plane-track-a.md`) stays a separate
   file, **not** merged into `improvement-135` — it's topically about the `architecture-map.html`
   visual tool, unrelated to nav-layer token-cost validation; forcing it into `improvement-135`
   would mix two unrelated "done" bodies of work under one issue for no benefit.
4. Update `backlog/BACKLOG.md`: drop `improvement-135` from the open priority table (its "135, 124"
   row becomes "124" alone), update this issue's own summary line to mention the absorbed
   items, remove the "### Top priority — improvement-135" section, add a `BACKLOG-ARCHIVE.md`
   entry for `improvement-135`'s closure.
5. Existing "improvement-135 item 5"/"item 3" mentions elsewhere in this file's Verification
   findings/Target plan (historical narrative explaining why Track B is gated) are left as-is —
   they're accurate as history, not rewritten wholesale.
6. No `DECISIONS.md`/ADR-index change needed.

Executed 2026-08-28 — content absorbed as planned below.

## Absorbed from `improvement-135` (items 3 and 5) — that issue is now closed, this is the live continuation

`improvement-135` validated the `.claude/nav/` layer built in `improvement-134`. Its items 1, 2,
and 4 are done and stay recorded in the now-closed
[`completed/tasks/improvement-135-ai-nav-layer-validation-and-adr-index-ci-check.md`](../completed/tasks/improvement-135-ai-nav-layer-validation-and-adr-index-ci-check.md).
Items 3 and 5 are copied here verbatim — this is the same open hypothesis as this issue's own
Track B/B2 (does a token-minimal generated layer actually save tokens), just applied to the
existing hand-authored `.claude/nav/` layer instead of a new one, and item 5's rule is what
directly gates Track B (see Finding 3 below).

### Item 3. Validate `context-loading.md` empirically — does it actually reduce reads — mechanism done, empirical answer pending real data

The original plan (a synthetic 3-5-task with/without experiment) was superseded by a recording
mechanism (`.claude/rules.md` "Final reports record real operational data in a fixed,
mechanically-parseable block" — every completed issue gets an `## Operational notes` block;
aggregated by `/sync-docs --full-audit` Step A5). The recording mechanism is live; the actual
empirical answer requires real `## Operational notes` data to accumulate across genuinely completed
tasks first, then gets reviewed at the next `/sync-docs --full-audit` run. Not answerable in one
sitting by design — a synthetic snapshot was the original plan's flaw, not evolving evidence.

### Item 5. Governing principle: do not add new `.claude/nav/*`-shaped content (including this issue's own Track B) until a real discovery gap appears

No new navigation file, no new metadata field, no expansion of `adr-index.md`'s schema (e.g. the
previously-rejected `Tags`/`Scope` field), and no new AI-context-loading projection (Track B's
L0-L5 layer) until item 3 shows the *existing* layer is pulling its weight, or a specific,
evidenced discovery failure demonstrates a gap the current layer can't cover. This mirrors
`CLAUDE.md`'s own "don't design for hypothetical future requirements" principle, applied to the
AI-navigation layer itself — a stale or speculative doc actively misleads, which is worse than no
doc at all.
...
- `backlog/tasks/` (open) vs `backlog/completed/tasks/` (closed) → `TRANSITIONAL` marker
...
- **Intent/History:** `.claude/nav/adr-index.md` (already generated) + `backlog/tasks/` frontmatter.
...
- **lifecycle + disposition** — from `DECISIONS.md` `Status:` fields (§6) and
  `backlog/tasks/` vs `backlog/completed/tasks/`
...
`backlog/tasks/`. **Confirmed directly, not installed in this repository's current state**:
...
| `backlog/BACKLOG.md`, `backlog/tasks/*.md`, `backlog/completed/tasks/*.md` | Tracked work, priority ranking | **KEEP entirely as-is; becomes a data source** (open issue → `TRANSITIONAL` lifecycle marker, per §6). | Already exactly the structured signal the lifecycle model needs — zero format change required. |
...
| ~~`.claude/skills/deep-review/`~~ (deleted 2026-08-25 — replaced by the `deep-review-orchestrator` agent, `.claude/agents/deep-review-orchestrator.md`, see `improvement-171`) | Two-mode (diff/full) evidence-verified review workflow | Row kept only for the historical rationale below (still applicable to its replacement): **not a migration target**, genuinely complementary — a qualitative/judgment review mechanism the model's mechanical extraction was never meant to replace. Three touchpoints: (1) the issues it files are already exactly the data §6's lifecycle join reads; (2) its core rule ("doc/code mismatch is itself a finding") is the same discipline as Invariant #10 (§2.5); (3) a future compliance lens is a natural future consumer of L3 once it exists. | Same reasoning, now pointed at the current file. |
...
## Planned — split Track A's execution history out to `completed/tasks/` (2026-08-28)

User request: split this issue into what's done (Track A, fully shipped) and what's not (Track B,
still gated on the item 5 governing rule, later absorbed from `improvement-135` — see below) —
move the done part into `backlog/completed/tasks/`.

**Plan:**
1. Move the entire `## Execution outcome — Track A (2026-08-04)` section below — everything from
   here through the end of the Bounded Contexts/Database ERD/SPI Map migration narrative, i.e.
   every dated sub-section up to (not including) the `## `05`-`08` mechanization work` pointer —
   verbatim into a new file, `backlog/completed/tasks/improvement-138-architecture-control-plane-track-a.md`.
   This is a pure historical execution log (every "not started" mention inside it already resolves
   to either a later sub-section in the same log, or to Track B's already-tracked gated status) —
   nothing in it is live, undone work.
2. Replace that whole span in this file with a one-line pointer to the new completed file.
3. Keep everything else in this file as-is: header, the 4 Verification findings, the full
   user-supplied "Target plan" (§0-§14 — still the live spec Track B implements), Testing
   strategy, the `05`-`08` → `improvement-143` pointer, Out of scope, Related. This issue stays
   open under its own number/title, now scoped to Track B (not started) plus the historical
   pointer.
4. Add a `✅ Done (2026-08-28): improvement-138 Track A` line to
   `backlog/completed/BACKLOG-ARCHIVE.md` under the relevant wave, referencing the new file.
5. No `DECISIONS.md`/ADR-index change (no ADR content touched) — no freshness-gate regen needed.

Executed 2026-08-28 — content moved as planned below.

## Execution outcome — Track A (2026-08-04) — moved to `completed/tasks/`

**Track A is fully done.** Its full execution history (Module Dependencies/SPI Map/Database
ERD/Bounded Contexts live migrations, all follow-up rounds, breadcrumb/back-button/toolbar fixes)
moved to
[`completed/tasks/improvement-138-architecture-control-plane-track-a.md`](../completed/tasks/improvement-138-architecture-control-plane-track-a.md)
on 2026-08-28, per explicit user request to split done vs. not-done work. This issue stays open,
scoped to Track B only (not started — see the 4 Verification findings above and §11's Track B
description in the plan below).
...
- Resolving `improvement-136` (paused separately) or completing the absorbed item 3's data
  accumulation (see "Absorbed from `improvement-135`" above) — this issue depends on but does not
  drive either.
...
- `improvement-135` (closed 2026-08-28) — items 1/2/4 done and archived there; items 3 and 5 were
  the only still-open parts and were absorbed verbatim into this file (see "Absorbed from
  `improvement-135`" above) — item 5's governing rule directly gates Track B (Finding 3); item 3's
