...
project's own standing rule that a plan lives in its tracked `backlog/tasks/<n>.md` file, not a
...
land in a harness-assigned file instead of the relevant `backlog/tasks/<n>.md` file — exactly the
...
5. ✅ DECIDED (2026-08-28), user-approved. `EnterPlanMode`'s plan file is harness-assigned — Claude
   cannot point it at a chosen path like `backlog/tasks/<n>.md` directly. So the rule becomes:
   after `ExitPlanMode` is approved, **duplicate that same plan text into the relevant
   `backlog/tasks/<n>.md` file** (when the task is tracked by one) — satisfies both the real,
...
`backlog/tasks/<n>.md` file when `/autopilot` was invoked for a tracked issue. The equivalent
