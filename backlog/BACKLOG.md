...
|---|---|---|---|
| Top | 🟡 | 178 | improvement-178 (filed 2026-08-28, carved out of `improvement-124` Batch 124-C per explicit user request) — **resequenced ahead of `improvement-175` per explicit user request** (once `ProviderProfileSaveService` is real, 175's fix gets re-evaluated against it, not designed blind). `AccountOverlay` (Name/Settings/Provider Profile tabs), new `ProviderProfileSaveService` in `marketplace-orchestrator` (mirrors `AdvertisementSaveService`), `AccessEvaluator.canEditUserAccount()`/`canViewUserAccount()`. Gates `improvement-175` and `improvement-124` Batch 124-D |
| Top | 🟡 | 175 | improvement-175 (filed 2026-08-28, carved out of `improvement-124` Batch 124-B2 per explicit user request) — now gated on `improvement-178`. Of its two original findings: the shared-sanitizer-utility finding was rejected during planning — verified against `platform-commons/CLAUDE.md`'s own "NOT ALLOWED: generic utils" rule, no other legal shared location existed at the time. The stale-id-during-concurrent-delete fix (`OptimisticLockingFailureException` when `dto.id() != null && before == null`) is planned but not yet implemented — both findings get re-checked once `improvement-178`'s `ProviderProfileSaveService` exists, since it opens a real orchestrator-layer option neither finding had available before |
...
### Top priority — improvement-178

| Issue | Origin | What |
|---|---|---|
| [improvement-178](issues/improvement-178-account-overlay-provider-profile-tab.md) | New (carved out of `improvement-124` Batch 124-C, filed 2026-08-28 per explicit user request) | **Resequenced ahead of `improvement-175` per explicit user request.** `AccountOverlay` (replaces `SettingsOverlay`) — Name/Settings/Provider Profile tabs via a plain `Tabs` component (not the removed 2-tab Content+Activity machinery per `ADR-067`), each with its own activity button opening `EntityActivityOverlay`. New `ProviderProfileSaveService` in `marketplace-orchestrator`, mirroring `AdvertisementSaveService`'s shape (2 direct ports + `TaxonAssignmentWriteService`) — the first real caller `ProviderProfilePort.save()` will ever have; confirmed directly no production caller exists today, the backend-only module shipped in Batch 124-B has had zero real-world effect since. New `AccessEvaluator.canEditUserAccount()`/`canViewUserAccount()` for moderator field-level readonly. Closes the "plain USER can't self-edit name" gap. Gates `improvement-175` and `improvement-124` Batch 124-D |

### Top priority — improvement-175

| Issue | Origin | What |
|---|---|---|
| [improvement-175](issues/improvement-175-shared-sanitizer-stale-id-delete-race.md) | New (carved out of `improvement-124` Batch 124-B2, filed 2026-08-28 per explicit user request) | **Now gated on `improvement-178`** — re-evaluate both findings once `ProviderProfileSaveService` is real. Finding 1 (shared `HtmlSanitizerUtil` in `platform-commons`) **rejected during planning** — verified against `platform-commons/CLAUDE.md`'s own "NOT ALLOWED: generic utils" rule (the `YoutubeUtil` precedent the original plan cited doesn't hold: zero external deps, unlike the two `HtmlSanitizerUtil` would pull in); `marketplace-orchestrator` was checked and rejected as a home *at the time* because no `ProviderProfileSaveService` existed yet — `improvement-178` removes that blocker, so this rejection needs re-checking, not assumed still final. Finding 2 (stale-id-during-concurrent-delete race) — plan drafted, not yet implemented: both services' `buildEntity()` would throw `OptimisticLockingFailureException` when `dto.id() != null && before == null` — but whether that check belongs in the starter or in the new `ProviderProfileSaveService` is exactly the open question `improvement-178` resolves first |
