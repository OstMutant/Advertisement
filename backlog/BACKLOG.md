...
|---|---|---|---|---|
| 1 | [improvement-074](issues/improvement-074-mockito-self-attach-dynamic-agent-slow-first-test.md) | New | Mockito dynamic self-attach adds a 40-90s one-time delay to whichever test runs first in each Maven test JVM fork | 🔴 highest — cheap, mechanical, zero-risk config fix (Mockito's own documented recommendation) removing a real, reproducible tax from every `mvn test` run |
| 2 | [improvement-056](issues/improvement-056-userpickerfield-inline-button-gap-and-pagination-bug.md) | Still open | `UserPickerField`'s `CallbackDataProvider` offset→page pagination correctness bug + inline search-button gap | 🟡 high ROI — item 2 is a real correctness bug at scale, not just cosmetic |
| 3 | [improvement-058](issues/improvement-058-taxon-assignment-audit-trail-missing.md) | Still open | Category/taxon assignment changes on advertisements are never recorded to the audit log — a stated ADR-019 goal never shipped | 🟡 high ROI — real, silent audit-trail gap, found via `/sync-docs --full-audit` |
| 4 | [improvement-066](issues/improvement-066-usersettingsrepository-missing-version-check.md) | Still open | `UserSettingsRepository.save()` has no optimistic-locking version check — settings from two tabs silently clobber each other | 🟡 high ROI — real, silent data-loss bug, breaks the "optimistic locking everywhere" principle (ADR-029) for one missed path |
| 5 | [improvement-048](issues/improvement-048-service-layer-test-coverage.md) | Still open | Cover `marketplace-app`'s non-UI service layer with unit tests | 🟡 high ROI — follow-up to already-done improvement-045 item 1, pattern already proven |
| 6 | [improvement-047](issues/improvement-047-integration-tests-ci-safety.md) | Still open | Keep `integration-tests` out of a plain `mvn install`/`mvn test`, Docker precheck, `.env` doc note | 🟡 high ROI — CI-safety, corrected version of an external PR review |
| 7 | [improvement-044](issues/improvement-044-shared-env-config-consolidation.md) | Still open | Consolidate DB/MinIO credentials duplicated across 4-5 files into root `.env` | 🟡 high ROI — pure drift-risk reduction |
| 8 | [improvement-061](issues/improvement-061-supportutil-tolong-silent-truncation-id-filter.md) | Still open | `SupportUtil.toLong(Double)` silently truncates fractional user-id filter input, no validation error shown | 🟡 high ROI — real, silent filter-correctness bug, found via direct code review |
| 9 | [improvement-079](issues/improvement-079-formoverlaymodehandler-activity-tab-duplication-and-userid-bug.md) | New | `UserFormOverlayModeHandler` passes the wrong `userId` (subject, not viewer) into `AuditActivityPanel` — a real, currently-masked audit-visibility bug, found while verifying a UI dedup proposal | 🟡 high ROI — traced through actual SQL, confirmed bug (not just duplication); currently masked by `User` entity ownership semantics but real |
| 10 | [improvement-060](issues/improvement-060-advertisementenrichservice-listallbytype-instead-of-findbyids.md) | Still open | `AdvertisementEnrichService.resolveCategoryNames()` loads the entire category table instead of `TaxonPort.findByIds()` | 🟡 medium ROI — no bug at today's volume, but a genuine memory-wall risk as the taxon table grows; fix is a one-call swap to an API that already exists |
| 11 | [improvement-067](issues/improvement-067-taxontranslationrepository-unbounded-in-clause.md) | Still open | `TaxonTranslationRepository.findAllByTaxonIds()` uses an unbounded `IN (:taxonIds)` clause | 🟡 medium ROI — same class of issue improvement-054 already fixed elsewhere, proven pattern, one method missed |
| 12 | [improvement-064](issues/improvement-064-s3storageservice-inputstream-not-closed.md) | Still open | `S3StorageService.upload()` never closes the `InputStream` it's given | 🟡 medium ROI — cheap, safe defensive fix; real severity uncertain (Vaadin's own upload machinery may already cover it) |
| 13 | [improvement-069](issues/improvement-069-attachment-s3-move-inside-db-transaction-orphans-on-rollback.md) | Still open | S3 file move (`commitTempUploadsQuiet`) happens inside `AdvertisementSaveService`'s DB transaction — orphaned S3 objects on rollback, never swept | 🟡 medium ROI — real data-integrity gap, confirmed structural (not yet an observed incident); orphans are permanently unswept by existing cleanup |
| 14 | [improvement-070](issues/improvement-070-attachmentsnapshotrepository-unsafe-array-cast-silent-swallow.md) | Still open | `AttachmentSnapshotRepository.extractUrls()` — unsafe `(String[])` cast wrapped in a silent catch-all with no logging | 🟡 medium ROI — real robustness/observability gap; a real failure here would look like "attachments vanished," not an error |
| 15 | [improvement-068](issues/improvement-068-attachment-audit-shows-uuid-not-original-filename.md) | Still open | Attachment audit/timeline diffs show S3 UUID keys instead of original filenames | 🟡 medium ROI — real, user-visible UX bug on every media change, no data-loss risk |
| 16 | [improvement-071](issues/improvement-071-taxonformoverlaymodehandler-raw-uicomponentfactory.md) | Still open | `TaxonFormOverlayModeHandler` uses a raw `UiComponentFactory<OverlayFormBinder>` — the other 3 form handlers already don't | 🟡 medium ROI — cheap, safe, proven pattern already used by 3 sibling handlers, just not adopted here yet |
| 17 | [improvement-075](issues/improvement-075-timeline-actor-filter-multi-select.md) | New | Timeline actor filter — support selecting multiple actors (add, not replace), removable chips | 🟡 medium ROI — real UX improvement, user-requested; moderate cross-layer scope (DTO/SQL trivial, UI is the bulk of the work) |
| 18 | [improvement-076](issues/improvement-076-advertisementcardview-redundant-stoppropagation.md) | New | `AdvertisementCardView` — remove redundant stopPropagation listeners (already registered by `BaseActionButton`) | 🟡 medium ROI — trivial, safe, zero-risk quick win |
| 19 | [improvement-077](issues/improvement-077-advertisementcardview-dead-updatedat-null-check.md) | New | `AdvertisementCardView` — remove dead `updatedAt == null` half of a check (`@LastModifiedDate` guarantees non-null) | 🟡 medium ROI — trivial, safe, zero-risk quick win |
| 20 | [improvement-082](issues/improvement-082-cardlightboxviewer-redundant-queryselector.md) | New | `CardLightboxViewer` — remove redundant `document.querySelector` JS calls, use held `iframe`/`videoEl` references directly; fixes a latent cross-instance control bug | 🟡 medium ROI — confirmed redundant code plus a genuine (low-probability) correctness bug |
| 21 | [improvement-078](issues/improvement-078-queryblock-filterrow-helper.md) | New | `QueryBlock` — extract `filterRow()` helper to collapse ~13 duplicated filter-row blocks across `AdvertisementQueryBlock`/`UserQueryBlock`/`TimelineQueryBlock` | 🟡 medium ROI — confirmed duplication, moderate structural change, additive/low-risk |
| 22 | [improvement-080](issues/improvement-080-taxonformoverlaymodehandler-locale-field-dedup.md) | New | `TaxonFormOverlayModeHandler` — collapse ~60-70 duplicated lines of EN/UK locale field wiring across 5 methods | 🟡 medium ROI — confirmed duplication, but touches binder validation — do carefully |
| 23 | [improvement-081](issues/improvement-081-lightbox-embedurl-and-iframe-attrs-duplication.md) | New | Extract duplicated embed-URL resolution + iframe security attributes from `AttachmentLightbox`/`CardLightboxViewer` | 🟡 medium ROI — verbatim-identical logic, low-risk extraction |
| 24 | [improvement-083](issues/improvement-083-advertisementcardview-thumbnail-click-no-op-when-attachment-port-unavailable.md) | New | `AdvertisementCardView` thumbnail — decide UX when `AttachmentPort` unavailable mid-session (narrower than originally framed — verified media fields are never denormalized, ADR-035) | 🟡 medium ROI — real but narrow edge case after correction |
| 25 | [improvement-025](issues/improvement-025-leaf-ui-components-plain-classes.md) | Still open | Convert ~17 stateless leaf UI widgets from `@SpringComponent` beans to plain Java classes | 🔵 larger tech-debt — no bug, 4 phased batches, brings code in line with an existing but unfollowed rule |
| 26 | [improvement-036](issues/improvement-036-actuator-structured-logging.md) | Migrated | Actuator + structured JSON logging | 🔵 larger tech-debt |
| 27 | [improvement-039](issues/improvement-039-dark-mode-lumo-tokens.md) | Migrated | Dark mode — step 1 (tokenization) done via improvement-037, step 2 (palette values + toggle) remains | 🔵 larger tech-debt — infra prerequisite already shipped |
| 28 | [improvement-040](issues/improvement-040-spring-boot-vaadin-minor-bump.md) | Migrated | Spring Boot 4.1.0 + Vaadin 25.2.1 bump | 🔵 larger tech-debt — routine, re-check latest versions before starting |
| 29 | [improvement-029](issues/improvement-029-docs-drift-guard-and-hooks.md) | Migrated | Docs-drift guard + incremental-compile Claude Code hooks | 🔵 larger tech-debt |
| 30 | [improvement-033](issues/improvement-033-quality-gate-skill-and-definition-of-done.md) | Migrated | `/quality-gate` skill + Definition of Done in rules.md — now unblocked (027/030/032 all done), likely reduces to just recording the Definition of Done since `/ci` already does the chaining | 🔵 larger tech-debt — cheap now that its prerequisites and most of its substance already exist |
| 31 | [improvement-062](issues/improvement-062-missing-readonly-transactional-on-port-impls.md) | Still open | `UserPortImpl`/`AdvertisementPortImpl`/`DefaultTaxonPort` have no `@Transactional(readOnly=true)`, unlike `DefaultAuditPort` | 🔵 larger tech-debt — consistency/read-correctness, no known bug from this today |
| 32 | [improvement-065](issues/improvement-065-settingspaginationservice-detach-not-guaranteed-on-session-expiry.md) | Still open | `SettingsPaginationService`'s `DetachListener` cleanup isn't guaranteed on abrupt session expiry | 🔵 larger tech-debt — follow-up on top of an already-shipped fix (ADR-028), not a fresh leak |
| 33 | [improvement-072](issues/improvement-072-uicomponentfactory-generics-design-debt.md) | Still open | Generics/type-safety design debt — `UiComponentFactory<T>`'s dual role, `AuditReadService`'s raw hook dispatch, `AuditDomainHookImpl.castIfKnown`'s missing type token | 🔵 larger tech-debt — needs a design decision, not a mechanical fix; no observed correctness bug from any of the three today |
| 34 | [improvement-038](issues/improvement-038-pg-trgm-title-index.md) | Migrated | `pg_trgm` GIN index on `advertisement.title` | 🔵 larger tech-debt — trigger-based (do as data volume grows), listed here only for completeness |
| 35 | [improvement-063](issues/improvement-063-playwright-stability-guard-async-init-components.md) | Still open | No "ready" signal for async-initialized custom components (`QuillEditor`, `AttachmentGallery`) — potential Playwright flakiness | ⚪ lowest priority — preventive, no flaky failure from this cause actually observed yet |
| 36 | [improvement-028](issues/improvement-028-minimal-ci-pipeline.md) | Migrated | Minimal CI pipeline (GitHub Actions: push/PR/nightly) | ⚪ lowest priority — improvement-059 (its former blocker) is done, but its own open questions (push authorization, `gh` CLI setup, clean-runner environment) are unrelated and still unresolved |
| 37 | [improvement-073](issues/improvement-073-rest-endpoint-infrastructure-test-seeding.md) | New | Add test-only, dev-gated REST endpoints for Playwright seeding (not this app's first REST controller — `HealthController` already exists, just not profile-gated/business-logic-invoking) | ⚪ lowest priority — deliberately sequenced after everything achievable on the existing codebase; new infrastructure surface area, not a fix |
| 29 | [improvement-035](issues/improvement-035-sql-seeding-for-playwright-spec-05.md) | Migrated | Service-layer-seed spec 05 via REST (raw SQL confirmed non-viable — breaks Test 6's timeline/audit assertions) — full e2e 11 min → ~7-8 min | ⚪ lowest priority — blocked on improvement-073; pure speed optimization, no coverage gap |
