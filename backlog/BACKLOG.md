...
|---|---|---|---|
| Top | 🔴 | 201 | improvement-201 — Two bundled findings (per explicit user direction, filed 2026-09-22). **Part 1:** Attachment upload content-type is never validated server-side, a confirmed stored-XSS-via-upload vector: `event.getContentType()` flows unvalidated from the client all the way into the S3 `PutObjectRequest`'s `Content-Type` metadata; the only restriction anywhere (`AttachmentUploadButton.setAcceptedMimeTypes`) is a trivially-bypassed client-side HTML5 `accept` attribute — real security bug, not tech debt. **Part 2:** City/Taxon vertical, 4 class pairs duplicated near-verbatim (~500+ lines) — the largest confirmed DRY debt in the codebase: `CityManagementView`/`TaxonManagementView`, `CityOverlay`/`TaxonOverlay`, `CityViewOverlayModeHandler`/`TaxonViewOverlayModeHandler` fully duplicated; `CityFormOverlayModeHandler`/`TaxonFormOverlayModeHandler` only partially deduped via the existing `LocaleTranslationForm<T>` precedent. Fix: generic `AbstractTaxonManagementView<T>` + analogous abstractions, mirroring the established `AbstractEntityOverlay<H>`/`LocaleTranslationForm<T>` pattern. Placed above every other backlog item |
...
| Top | 🟡 | 191 | improvement-191 — UI/UX Modernization Map, reconciled with improvement-188/096/129: a second external "UI/UX MODERNIZATION v2" directive verified accurate on its ArchUnit-boundary and Playwright-visual-QA claims, but its theme-baseline investigation duplicates improvement-188's already-answered "lean no" Aura finding and its AdvertisementCard/mobile scope overlaps improvement-129/096 — scoped down to the genuinely new parts (Query Block, Header/Tabs, surface system, typography, color, dialogs, Timeline, User UI, attachments, Provider Profile) plus the Map-then-approval process. `improvement-188` itself is now fully done (2026-09-17) — the wait on its Tasks C/D landing is satisfied. **Still waits on `improvement-129`'s open design questions being resolved** — sequenced last for this reason |
| Nice to have | — | 096, 129, 036, 065, 028, 133, 172, 142, 177, 185, 196, 197, 198 | everything else — no internal priority order, pick up opportunistically |
| (Under reconsideration) | ⚪ | 039 | Dark mode — a real implementation attempt failed (UI wiring confirmed broken); deprioritized to the very bottom 2026-09-16 pending a decision on whether the feature is even wanted, not just how to build it |
...
### Top priority — improvement-201

| Task | Origin | What |
|---|---|---|
| [improvement-201](tasks/improvement-201-attachment-upload-content-type-not-validated-server-side.md) | New (two findings bundled into one task per explicit user direction, both found via an external code-review-style pass, re-verified end-to-end directly against current source, filed 2026-09-22) | **Part 1:** `AttachmentGallery.buildUploadHandler()`'s client-supplied `contentType` flows unvalidated through `AttachmentMediaService`/`AttachmentService`/`S3StorageService` straight into the S3 object's `Content-Type` metadata — no server-side whitelist anywhere. The only restriction, `AttachmentUploadButton.setAcceptedMimeTypes(...)`, is a client-side HTML5 `accept` attribute, trivially bypassed via a direct multipart POST — confirmed exploitable stored-XSS-via-upload vector. **Part 2:** City/Taxon vertical — 4 class pairs (`CityManagementView`/`TaxonManagementView`, `CityOverlay`/`TaxonOverlay`, `CityViewOverlayModeHandler`/`TaxonViewOverlayModeHandler`, `CityFormOverlayModeHandler`/`TaxonFormOverlayModeHandler`) duplicated near-verbatim (~500+ lines), the largest confirmed DRY debt in the codebase — fix via a generic `AbstractTaxonManagementView<T>` + analogous abstractions, extending the existing `AbstractEntityOverlay<H>`/`LocaleTranslationForm<T>` precedent. See the issue file for both parts' full approach, including a mandatory `/code-review` pass before Playwright verification |

### Top priority — improvement-199

| Task | Origin | What |
|---|---|---|
| [improvement-199](tasks/improvement-199-f05-contact-reveal-click-analytics.md) | New (private product roadmap Phase 2, F-05 + F-11a; filed 2026-09-22; user-requested Top placement, 2026-09-22) | Structured phone/Telegram/Viber contact block on listings and master profiles, gated behind a counted "Show contacts" click-to-reveal interaction (server round-trip, contacts never in the initial DOM) — an append-only `contact_view` event table plus a per-master "Contact views: N this month" counter. Plus F-11a: a dedicated "request board" framing over the already-shipped `AdKind.REQUEST` catalog filter, wired to the same contact-reveal interaction — no response entity, per the spec's own simplified MVP scope. Depends only on F-04 (provider profile), already shipped as `improvement-124`. See the issue file for full scope and the reveal rate-limiting approach |

### Top priority — improvement-200

| Task | Origin | What |
|---|---|---|
| [improvement-200](tasks/improvement-200-f06-reviews-ratings.md) | New (private product roadmap Phase 3, F-06; filed 2026-09-22; user-requested Top placement, 2026-09-22) | New `review-spring-boot-starter`: rating (1-5) + sanitized text review on master profiles, one per author per master, edit window, master response, moderation via the existing management area. Denormalized avg rating + count on the master profile, updated transactionally. Blocked on `improvement-199` landing first (Phase 2 before Phase 3, per roadmap order); depends only on F-04 (provider profile), already shipped. See the issue file for full scope |

### Top priority — improvement-186

| Task | Origin | What |
|---|---|---|
| [improvement-186](tasks/improvement-186-rest-hypermedia-media-subresource-and-patch-support.md) | New (carved out of `improvement-183` item 9, filed 2026-09-12) | Three related, independently-sizable pieces designed together: (1) a dedicated advertisement media sub-resource (`POST`/`GET`/`DELETE`) replacing today's no-media-via-REST gap; (2) project-wide `PATCH` support via `JsonNullable` tri-state fields, for true partial updates anywhere a full `PUT` replace is currently the only option; (3) REST hypermedia (HATEOAS/HAL `_links`, a `GET /api/me` entry point, Swagger-side action-discovery tooling) so a client can discover what it can do next from a response instead of guessing. Design fully worked out and ready — the largest single piece of work in the Top list, sequenced after the smaller/quicker items above. See the issue file for the full design and rejected alternatives |

### Top priority — improvement-138

| Task | Origin | What |
|---|---|---|
| [improvement-138](tasks/improvement-138-architecture-control-plane.md) | New (user-supplied task, 2026-08-04, "FINAL VISION v2" — supersedes an unseen `architecture-observability-vision.md` v1) | A generated, evidence-linked model of the repo (code, tests, pipelines, docs) read through two projections: a token-minimal AI layer (L0-L5) and a Cytoscape.js human visual explorer. **Track A** (visual control from already-structured sources) shipped, execution history archived to `completed/tasks/improvement-138-architecture-control-plane-track-a.md`. This issue's live scope is now **Track B** (ArchUnit-based contract/test-coverage model + the AI-token-savings hypothesis, gated by its own B2 "stop" measurement) plus **`improvement-135`'s absorbed items 3/5** (moved in 2026-08-28, since it's the same "does a nav layer save tokens" question, just for the existing hand-authored `.claude/nav/` layer instead of Track B's new generated one) — `improvement-135` had nothing else still-open and is now closed. **Self-gated by its own governing rule — do not start until the empirical token-savings data exists**, listed last in Top for this reason. See the issue file's "Absorbed from `improvement-135`" section for the live governing rule and pending measurement. |

### Top priority — improvement-191

| Task | Origin | What |
|---|---|---|
| [improvement-191](tasks/improvement-191-ui-modernization-map-reconcile-188-096-129.md) | New (evaluated an external "UI/UX MODERNIZATION v2" directive against this codebase, filed 2026-09-15; user-requested top-of-backlog placement, 2026-09-15; execution order confirmed 2026-09-15) | The directive's own claims were verified against real code: its ArchUnit-boundary claim (`marketplace_app_must_not_depend_on_platform_commons_spi_directly`) and its "geometry-only Playwright pass hid a visually wrong result" lesson (`improvement-126`) both check out. But its theme-baseline investigation (§0/§16) duplicates `improvement-188` Task A's already-delivered "lean no" Aura go/no-go, and its AdvertisementCard (Wave 1) plus mobile/responsive (§13) scope overlaps `improvement-129` and `improvement-096`. Scoped down to what isn't already owned: Query Block, Header/Tabs, surface system, typography, color discipline, vertical rhythm, dialogs, Timeline, User UI, attachments, Provider Profile visual refinement — plus the document's own Map-then-human-approval process before any implementation. `improvement-188` closed in full 2026-09-17 (Tasks 0/A/C/D all done). **Still waits on `improvement-129`'s open design questions being resolved** — sequenced last in Top for this reason. See the issue file for full rationale |
...
| [improvement-196](tasks/improvement-196-rest-api-rate-limiting.md) | New (user-supplied audit finding, verified against code, filed 2026-09-17) | No rate limiting on any `marketplace-rest-api` write endpoint (Advertisement/Taxon/ProviderProfile create/update/delete, API-key issuance/revocation) — a self-issued bearer key (no approval step) currently faces unlimited request volume. Reuse the existing `FailureRateLimiter` (`platform-commons`) via a single `/api/**` filter rather than a third hand-rolled counter; needs a limit-shape decision (per-key budget, stricter on writes) before implementation |
| [improvement-197](tasks/improvement-197-rest-api-contract-bugs.md) | New (targeted REST/API+security delta-audit pass, verified against running app + code, filed 2026-09-17) | Two real REST contract bugs: (1) omitting the documented-optional `If-Match` header on Advertisement/ProviderProfile/Taxon `PUT`/`DELETE` unconditionally throws `412` (SQL `version = NULL` never matches) instead of working as "optional" implies — confirmed untested by any existing test; (2) pagination `Link` header URLs are missing the resource path entirely (`http://host?page=1` instead of `http://host/api/advertisements?page=1`), confirmed live via curl — the one existing test only checks `containsString("rel=\"next\"")`, never the actual URL |
| [improvement-198](tasks/improvement-198-playwright-test-isolation-and-parallel-workers.md) | New (carved out of `improvement-195` Phase 7, filed 2026-09-22) | Whether this project's serial/shared-state Playwright design (`fullyParallel: false`/`workers: 1`, "each spec depends on state left by the previous one") is worth changing for standard-practice test isolation and retryability, plus whether `workers: 2` is now viable given `improvement-073`'s REST-based seeding already removed spec 06's own browser-UI load — open investigation, no design chosen yet; also holds an open placeholder section for future Playwright feature work |
...
### Under reconsideration — improvement-039

| Task | Origin | What |
|---|---|---|
| [improvement-039](tasks/improvement-039-dark-mode-lumo-tokens.md) | Migrated; its remaining scope was tracked as `improvement-188` Task B 2026-09-15/16, moved back here in full 2026-09-16 | Dark mode — step 2 (palette values + toggle); step 1 shipped via improvement-037. A real implementation attempt exists on the unmerged `improvement-039` branch: backend layer (theme_preference column, Port/Service/Repository) passes its own tests, but the UI wiring (header toggle, flash-prevention listener) is confirmed broken via manual testing — neither saving nor switching the theme works. Deprioritized to the very bottom of the whole backlog per explicit user request: **before resuming, decide whether this feature is actually wanted**, given the real effort the first attempt showed this needs |
