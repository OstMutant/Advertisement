...
|---|---|---|---|
| **Top** | 🔴 | 145 | improvement-145 (filed 2026-08-06, **highest priority per explicit user request — first to execute**) — the "L3" token-efficiency gap: on-demand ADR full-text extraction (`node scripts/architecture/md-to-decisions-json.js --extract <module> <ADR-NNN>[,...]`, raw markdown output) so Claude can read just the ADRs a task needs instead of a whole `DECISIONS.md` (measured: `marketplace-app/DECISIONS.md` is 3580 lines / 72 ADRs) — not started |
| Top | 🟡 | 146 | improvement-146 (filed 2026-08-06, split out from `improvement-144` once that issue's other steps were done) — the companion-server on-demand Sonar/ArchUnit refresh trigger. **Priority tentative — not yet decided whether this will be built**; positioned here per explicit request, not a commitment. Port number and error-payload shape still undecided |
...
### Top priority — improvement-145

| Issue | Origin | What |
|---|---|---|
| [improvement-145](issues/improvement-145-adr-extraction-token-efficiency.md) | New (filed 2026-08-06, highest priority per explicit user request) | On-demand ADR full-text extraction — extends `scripts/architecture/md-to-decisions-json.js` with `--extract <module> <ADR-NNN>[,...]` (raw markdown output, reuses existing parsing, no new generated file/freshness gate needed). Addresses `scripts/architecture/DECISIONS.md`'s long-standing "AI-layer L3" open goal: `docs/ai/adr-index.md`/`context-loading.md` already tell Claude *which* ADRs are relevant, but reading them today means opening the whole `DECISIONS.md` (measured: `marketplace-app/DECISIONS.md` is 3580 lines/72 ADRs) — not started |

### Top priority — improvement-146

| Issue | Origin | What |
|---|---|---|
| [improvement-146](issues/improvement-146-code-quality-refresh-companion-server.md) | Split out from `improvement-144` (2026-08-06) once that issue's other steps were done | The companion-server on-demand Sonar/ArchUnit refresh trigger — full plan carried over verbatim from `improvement-144`'s Part B. **Priority tentative — whether to build this at all is still an open question**, not just an implementation detail; port number and error-payload shape also undecided |
