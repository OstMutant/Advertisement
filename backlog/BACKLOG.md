...
|---|---|---|---|
| Top | 🟡 | 151 | improvement-151 (filed 2026-08-12, explicit user request to rank at the very top) — `unit-tests.sh` never installs to `~/.m2`, so `integration-tests.sh`'s own staleness check (ADR-007) always finds every module stale right after `unit-tests.sh` just compiled them, and redundantly reinstalls. Fix: a new `scripts/build.sh` (`mvn install -DskipTests` once for the shared reactor modules), called first by `run-all-tests.sh`, so both existing scripts degrade to their own already-built-in fast paths ("nothing to compile" / "nothing stale") with no changes to either script itself |
