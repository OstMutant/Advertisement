...
|---|---|---|---|
| Top | 🟡 | 199 | improvement-199 — F-05 contact reveal + click analytics (private product roadmap Phase 2): structured phone/Telegram/Viber contact block, each channel independently revealed/counted, via a new dedicated `contact-spring-boot-starter` (`contact_info` + `contact_view` tables, generic over entity) — the platform's core demand-signal metric per `private/roadmap.md`. Next unblocked item in the roadmap (F-04 already shipped); user-requested Top placement, 2026-09-22. F-11a request board deferred indefinitely 2026-09-24 (dropped from this task's scope — no trigger set) |
...
| Task | Origin | What |
|---|---|---|
| [improvement-199](tasks/improvement-199-f05-contact-reveal-click-analytics.md) | New (private product roadmap Phase 2, F-05; filed 2026-09-22; user-requested Top placement, 2026-09-22; F-11a deferred indefinitely 2026-09-24) | Structured phone/Telegram/Viber contact block on listings and master profiles, each channel independently gated behind its own counted click-to-reveal (server round-trip, contacts never in the initial DOM) — a new dedicated `contact-spring-boot-starter` owning `contact_info` (what to show) and `contact_view` (per-channel reveal events), exposed via `ContactPort`, kept fully decoupled from both `provider-profile-spring-boot-starter` and `advertisement-spring-boot-starter`. Depends only on F-04 (provider profile), already shipped as `improvement-124`. See the issue file for full scope, UI mockups, and the reveal rate-limiting approach |
