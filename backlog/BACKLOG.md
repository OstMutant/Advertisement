...
|---|---|---|---|
| Top | 🟡 | 193 | improvement-193 — External SOLID/DRY review batch, 5 confirmed items: `ApiExceptionHandler`'s generic `IllegalStateException`→429 mapping, `TaxonService.update()`'s redundant manual `updatedAt`, `AttachmentService`'s 15-method SRP spread, `UiComponentFactory<T>`'s remaining unchecked cast (extends already-decided `improvement-072`/ADR-058, not a redo), and the near-exact `CityFormOverlayModeHandler`↔`TaxonFormOverlayModeHandler` duplication `improvement-080` didn't cover. 2 of the review's 7 original findings excluded — one false positive (stale README claim), one already deliberately decided differently (`improvement-028`). Not started yet |
| Top | 🟡 | 190 | improvement-190 — `.claude/` rule system self-consistency audit: mechanical `paths:` glob check + a scoped semantic review of `.claude/rules.md`/`.claude/rules/*.md`/`CLAUDE.md` for contradictions, dead rules, and duplication — narrowed down from a 27-section external "forensic audit" directive evaluated as disproportionate; cheap, low-risk, deterministic — a natural pairing with 192's quick-win Task D+E. Not started yet |
| Top | 🟡 | 188 | improvement-188 — Theme modernization: **Task 0 done** (Vaadin 25.2.3→25.2.8 + 5 other dependency bumps, verified 766/766 + 63/63). **Task A done** (Aura pilot — real `@StyleSheet(Aura.STYLESHEET)` build-time setup, go/no-go delivered: lean "no", minimal visual differentiation for this app's own heavily-customized token system, no forced future Aura migration since Lumo stays fully supported). Tasks B/C/D (light-dark() dark mode, oklch()/color-mix() accent tokens, @layer cascade) ready to start, no blockers |
...
| Top | 🟡 | 191 | improvement-191 — UI/UX Modernization Map, reconciled with improvement-188/096/129: a second external "UI/UX MODERNIZATION v2" directive verified accurate on its ArchUnit-boundary and Playwright-visual-QA claims, but its theme-baseline investigation duplicates improvement-188's already-answered "lean no" Aura finding and its AdvertisementCard/mobile scope overlaps improvement-129/096 — scoped down to the genuinely new parts (Query Block, Header/Tabs, surface system, typography, color, dialogs, Timeline, User UI, attachments, Provider Profile) plus the Map-then-approval process. **Waits on `improvement-188`'s Tasks B/C/D landing and `improvement-129`'s open design questions being resolved** — sequenced last for this reason |
...
### Top priority — improvement-193

| Task | Origin | What |
|---|---|---|
| [improvement-193](tasks/improvement-193-external-review-batch-exception-taxon-attachment-uicomponentfactory-city-taxon.md) | New (evaluated an external SOLID/DRY/best-practices review's 7 findings against current code, filed 2026-09-15; user-requested top-of-backlog placement, 2026-09-15) | Each of the 7 source findings verified directly against `main` before filing. 2 excluded: finding 1 (README "no UI yet" for Provider Profile) is a false positive — no such claim exists in the current README; finding 2 (no CI pipeline) is already tracked as `improvement-028`, deliberately deprioritized with its own recorded reasoning, not relitigated here. The remaining 5 (kept as 3-7 to match the source numbering): (3) `ApiExceptionHandler`'s generic `IllegalStateException`→429 mapping — correct only by accident since the login rate-limiter is currently its sole source; (4) `TaxonService.update()` manually sets `updatedAt` despite `@LastModifiedDate` already covering it; (5) `AttachmentService`'s 15 public methods span 4 real concerns with no section-header grouping yet, let alone a class split; (6) `UiComponentFactory<T>.build()` still has an unchecked cast — narrower than the source review assumed, since `improvement-072`/ADR-058 already bounded `T extends Configurable<T, ?>` in 2026-07; the gap is `P` not being linked too; (7) `CityFormOverlayModeHandler`/`TaxonFormOverlayModeHandler` are still near-exact 283/282-line duplicates — `improvement-080` fixed Taxon's own internal duplication but never touched this cross-file copy. See the issue file for full rationale and per-item approach |

### Top priority — improvement-190

| Task | Origin | What |
|---|---|---|
| [improvement-190](tasks/improvement-190-claude-rules-self-consistency-audit.md) | New (evaluated an external 27-section forensic-audit directive against this codebase, filed 2026-09-15; user-requested top-of-backlog placement, 2026-09-15; execution order confirmed 2026-09-15) | A 27-section "PRINCIPAL ARCHITECT DIRECTIVE" (governance + architecture + security + context-bloat audit in one pass, including gated git-commit-bypass testing) was submitted and evaluated as disproportionate and largely duplicative of existing mechanisms (ADR index, `/review` → `deep-review-orchestrator`, "one fact → one canonical home"). Narrowed to the genuinely open gap: nothing currently re-verifies the `.claude/` rule system's own internal consistency the way `generate-adr-index.sh` re-verifies `DECISIONS.md`. Approach: (1) mechanical `paths:` glob check against the real file tree, (2) scoped semantic review of `.claude/**` + `CLAUDE.md` only for contradictions/dead rules/duplication — explicitly excludes Java architecture/security/domain scope already covered by `/review`. Cheap, deterministic, low-risk — paired right after improvement-192's quick-win Task D+E. See the issue file for full rationale |

### Top priority — improvement-188

| Task | Origin | What |
|---|---|---|
| [improvement-188](tasks/improvement-188-theme-modernization-aura-modern-css.md) | New (evaluated an external "THEME MODERNIZATION" directive against this codebase, filed 2026-09-15; user-requested top-of-backlog placement, 2026-09-15) | Four related approaches. **Task 0 done:** Vaadin `25.2.3`→`25.2.8` + 5 other dependency bumps, two real convergence conflicts fixed, verified 766/766 backend tests + 63/63 Playwright. **Task A done:** real `@StyleSheet(Aura.STYLESHEET)` build-time pilot (the originally-assumed `?theme=aura` runtime switch doesn't actually exist in Vaadin Flow — corrected mid-task); go/no-go delivered — lean "no", this app's own 57 `--app-*` custom tokens already dominate visible chrome so a base-theme swap alone shows minimal visual difference, and no future Aura migration is forced (Lumo stays fully supported; only the deprecated `@Theme`/`theme.json` *mechanism* needs eventual migration, tracked separately as `improvement-116`). Tasks B (`light-dark()` dark mode) / C (`oklch()`/`color-mix()` accent tokens) / D (`@layer` cascade) not started, ready to pick up |
