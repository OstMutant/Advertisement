...
validates whether improvement-134's `.claude/nav/` layer earns its cost — item 1 (ADR-index
...
|---|---|---|---|
| Top | 🟡 | 157 | improvement-157 (filed 2026-08-17, split out of improvement-152 Part C) — SPI Interface Details table redesign: split Callers/Implemented By into two tables, group by Module → Class → Method instead of today's single class-level table. `improvement-156` (shipped 2026-08-27) now provides real method-level caller/implementor data (`spiEdges`) this redesign needs; the currently-shipped two-table shape (grouped by Interface, no Method column) diverges from this issue's original Module → Class → Method spec — open questions: add the Method column (data already available), regroup by Module, DI-wiring-only-caller display (now largely moot — real data no longer produces that false positive), generic-interface type-argument display |
...
| Nice to have | — | 073 → 035, 096, 129, 036, 039, 065, 114, 063, 028, 130, 131, 133, 172 | everything else — no internal priority order, pick up opportunistically |
