...
|---|---|---|---|
| Top | 🟡 | 192 | improvement-192 — Agentic governance hardening: start with the safe, read-only parts first (Task D+E — role/tool-scope audit of all 6 real `.claude/agents/*.md` files, report only, zero risk), then Task A — two confirmed commit-gate bypasses (substring-match evasion via `git -c .../--git-dir=... commit`, and an unscoped keyword-triggered `/tmp/commit-approved` marker) found by direct inspection of `.claude/settings.json`'s existing `UserPromptSubmit`+`PreToolUse` hook pair, produced as a diff only (protected-path change, needs its own approval before applying). Also carries Task B (`plansDirectory` — unverified) and Task C (>3-specialist wave threshold, no current behavior change). Not started yet |
...
### Top priority — improvement-192

| Task | Origin | What |
|---|---|---|
| [improvement-192](tasks/improvement-192-agentic-governance-hardening-commit-hook-review-waves-agent-audit.md) | New (evaluated an external "AGENTIC GOVERNANCE HARDENING" directive against this codebase, filed 2026-09-15; user-requested top-of-backlog placement, 2026-09-15; execution order confirmed 2026-09-15) | The directive's Task A assumed the `git commit`/`push` gate is prose-only — checked directly against `.claude/settings.json` and found false: a real `UserPromptSubmit`+`PreToolUse` hook pair already exists. But reading that hook's shell logic surfaced two confirmed bypasses: a substring-match evasion (`git -c user.name=x commit` / `git --git-dir=... commit` contain no literal `"git commit"` substring, so the block never fires) and an unscoped, keyword-triggered `/tmp/commit-approved` marker (no session-id binding, armed by the bare word "commit" appearing anywhere in a prompt). Task A rescoped to hardening the existing hook rather than designing a new one. Also carries: Task B (`plansDirectory` support — unverified, needs a real docs/changelog check), Task C (>3-parallel-specialist wave threshold for the Review Factory — today's count is exactly 3, so no behavior change yet), Task D+E (role-specificity + tool-scope report-only audit of all 6 real `.claude/agents/*.md` files). **Start with Task D+E** (read-only, zero risk), **then Task A** (diff only, protected-path change needs its own approval). See the issue file for full rationale |
