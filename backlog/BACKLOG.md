...
| Top | 🟡 | 114 | improvement-114 — SonarQube's `new_coverage` quality gate condition always reads 0% because JaCoCo is never wired into the scan, structurally guaranteeing the gate can't pass on that condition regardless of real test coverage; re-confirmed live 2026-09-08 (moved to Top per explicit request after re-verifying the gap is still real) |
| Nice to have | — | 035, 096, 129, 036, 039, 065, 063, 028, 130, 131, 133, 172, 142, 177, 181, 185 | everything else — no internal priority order, pick up opportunistically |
...
### Top priority — improvement-114

| Issue | Origin | What |
|---|---|---|
| [improvement-114](issues/improvement-114-sonar-jacoco-coverage-not-wired.md) | New (found running Sonar after improvement-113); moved to Top 2026-09-08 per explicit request, re-verified still real the same day | SonarQube's `new_coverage` quality gate condition always reads `0.0%` (threshold ≥80%) because no JaCoCo report is ever fed to the scanner — this condition structurally cannot pass regardless of how well-tested new code actually is, on every scan, until `jacoco-maven-plugin` is wired into the unit/integration test runs and `sonar.coverage.jacoco.xmlReportPaths` points at the aggregated report. See the issue file for the two separable sub-problems (service-layer JUnit coverage vs. Playwright-verified UI code) and the suggested fix steps. |
...
| [improvement-181](issues/improvement-181-module-list-auto-discovery.md) | New (found while implementing `improvement-073`'s module split, filed 2026-09-03) | Adding a new Maven module currently requires manually updating hand-maintained module-name lists in `scripts/build-and-test/build.sh`, `integration-tests/run.sh`, and `ArchitectureRulesTest` — confirmed one was missed and silently skipped a new module's tests; auto-discover from root `pom.xml` instead |
| [improvement-185](issues/improvement-185-full-javadoc-integration-architecture-map.md) | New (exploratory chat discussion, filed 2026-09-08, follow-up to this session's per-class purpose-tooltip work) | Whether/how to integrate full standard Javadoc (method-level `@param`/`@return`/`@throws`, cross-referenced) into architecture-map, beyond today's one-line class-purpose hover tooltip — three undecided candidates (external generated site, iframe embed, or extending the lightweight in-house extraction into a new class-detail screen), no design chosen yet |
