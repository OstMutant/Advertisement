...
|---|---|---|---|
| Top | 🟡 | 188 | improvement-188 — Theme modernization: **Task 0 done** (Vaadin 25.2.3→25.2.8 + 5 other dependency bumps, verified 766/766 + 63/63). **Task A done** (Aura pilot — real `@StyleSheet(Aura.STYLESHEET)` build-time setup, go/no-go delivered: lean "no", minimal visual differentiation for this app's own heavily-customized token system, no forced future Aura migration since Lumo stays fully supported). **Task C done** (accent-primary's 9 derived tokens + `-rgb` helper converted to CSS Relative Color Syntax `oklch(from ...)`/`rgb(from ...)`, exact reproduction of prior hex values, 63/63 Playwright, ADR-083). Task D (`@layer` cascade) ready to start, no blockers — dark mode moved back to `improvement-039` (2026-09-16) after a failed implementation attempt, see that issue |
...
| Nice to have | — | 035, 096, 129, 036, 065, 063, 028, 133, 172, 142, 177, 185 | everything else — no internal priority order, pick up opportunistically |
