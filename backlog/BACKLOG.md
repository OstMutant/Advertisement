...
| [improvement-177](issues/improvement-177-plan-mode-approval-rule-rollout.md) | New (carved out of `improvement-176` finding 4, filed 2026-08-28) | Replace the project-wide "Approval Rule" in `.claude/rules.md` (governs every conversation, not just `/autopilot`) with real Plan Mode (`EnterPlanMode`/`ExitPlanMode`) — same migration `improvement-176` finding 4 already shipped for `autopilot.md` alone (now closed, `completed/issues/`), but at project scope. The narrower version has landed and been used, so its real, verified `EnterPlanMode`/`ExitPlanMode` mechanics (plan file, no plan-text parameter, built-in approval request) are available to build on directly |
