...
|---|---|---|---|
| Top | 🔴 | 195 | improvement-195 — best-practices reference doc (Java/SOLID-DRY, JUnit, Playwright, Bash, docs, CI/CD — each checked against a real example already in this codebase) plus a set of concrete, already-verified fixes the same audit surfaced: a reflection-based test hack this project's own ADR-008 forbids, a redundant `Thread.sleep` in an integration test, a dead Playwright trace-on-retry config, a live YouTube dependency in e2e, two real ShellCheck-flagged bugs, inconsistent `set -euo pipefail`/shebang usage across scripts, and stale `improvement-NNN` citations left in 6 current `DECISIONS.md` files. User-requested top-of-backlog placement, 2026-09-17 — sequenced first |
...
| Nice to have | — | 035, 096, 129, 036, 065, 063, 028, 133, 172, 142, 177, 185, 196, 197 | everything else — no internal priority order, pick up opportunistically |
...
### Top priority — improvement-195

| Task | Origin | What |
|---|---|---|
| [improvement-195](tasks/improvement-195-best-practices-reference-and-audit-fixes.md) | New (real cross-cutting audit against the actual codebase, filed 2026-09-17; user-requested top-of-backlog placement, 2026-09-17) | Phase 0: decide where a project-grounded best-practices reference doc lives (candidates: a new `docs/best-practices.md`, a new doc-standards skill, or split per-module), each kept practice paired with a short sourced description pulled from an authoritative external source, not just paraphrased from memory — also surfaces a real gap in `module-readme-standards`/`app-readme-standards` if neither already covers where a repo-wide (not single-module) reference doc belongs. Phases 1-5: concrete, already-verified fixes the same audit found — a reflection-based test hack in `TimelineViewTest` (violates this project's own ADR-008), a redundant `Thread.sleep` in `AttachmentRepositoryTest`, a dead `trace: 'on-first-retry'` config paired with `retries: 0` in `playwright.config.js`, a live YouTube embed dependency in e2e tests, no ESLint in `playwright/`, two real ShellCheck-flagged bugs (`generate-architecture-model.sh`'s array-vs-string mixup, `build.sh`'s unguarded `rm -rf`), inconsistent `set -euo pipefail`/shebang usage and no ShellCheck CI gate across `scripts/`, stale `improvement-NNN` citations left in 6 current `DECISIONS.md` files (violating this project's own `improvement-141` rule), and an explicit, documented acknowledgment of the missing security-scanning gate (tied to the already-open `improvement-028` hosted-CI deferral). See the issue file for full per-phase detail |
