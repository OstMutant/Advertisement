...
|---|---|---|---|
| **Top** | 🔴 | 143 | improvement-143 (filed 2026-08-05, extracted from improvement-138 into a self-contained batch, **highest priority per explicit user request — first to execute**) — seven pieces on `docs/architecture/05-08-*.md`, ending with **all four files deleted** (corrected 2026-08-06, content captured in improvement-142 first): SonarQube metrics on the Module page, ArchUnit coupling/instability/abstractness metrics, 6 new `@ArchTest` rule candidates, live coupling checks + `07`'s 4 mechanizable pieces, `05`/`06`/`07` deletion after their live pieces ship, `08` deletion outright (no mechanizable content at all), and a later move of `architecture-map.html`/`architecture-model.json` into `docs/architecture/`. Pieces 1/2/4/5/7 are concretely specified and independently runnable (candidates for `/autopilot`); pieces 3/6 still have open decisions, not autopilot-ready yet |
...
| Top | 🟡 | 142 | improvement-142 (filed 2026-08-05, continuation of improvement-138 once that file's own scope closed) — `bounded-contexts.md` is already deleted; carry its four genuinely-unique sections (Domain Details prose, Shared Kernel category breakdown, Domain Independence, Risks & Future Considerations — draft content captured in the issue file) into `generate-architecture-model.sh` as hand-preserved static content (same 01/02/04 discipline); also the running tracker for further `architecture-map.html` follow-ups going forward |
...
### Top priority — improvement-143

| Issue | Origin | What |
|---|---|---|
| [improvement-143](issues/improvement-143-architecture-docs-05-08-mechanization-batch.md) | New (filed 2026-08-05, extracted from improvement-138 into a self-contained batch, per user request to make it an independently-runnable/autopilot-able unit; set to highest priority, first to execute, per explicit user request) | Seven planned-but-not-implemented pieces covering `docs/architecture/05-08-*.md`, each verified against a real running system or real file content before being written down: (1) SonarQube metrics on the Module page — verified live against a real running SonarQube server; (2) ArchUnit `ArchitectureMetrics.componentDependencyMetrics()` for real Efferent/Afferent Coupling, Instability, Abstractness per module — no extra dependency needed, already in `marketplace-app/pom.xml`; (3) 6 new `@ArchTest` rule candidates closing real gaps between `.claude/rules.md`/module `CLAUDE.md` text and the existing 8-rule `ArchitectureRulesTest.java`; (4) live coupling checks on the Module Dependencies page + `07-risk-report.md`'s 4 genuinely mechanizable pieces (corrected after an earlier "confirmed duplicate" claim was checked row-by-row and found wrong); (5) delete `05-sequence-diagrams.md` and its tool wiring — no tool found makes it live without a much bigger OpenTelemetry effort, full content preserved in `improvement-142` first; (6) later move `architecture-map.html`/`architecture-model.json` into `docs/architecture/`, sequenced last; (7) delete `08-scorecard.md` outright — no mechanizable content at all, pure editorial 1-10 scoring, content captured in `improvement-142` for the record only. **Corrected decision (2026-08-06): `06`/`07` are also deleted in full** (not kept in reduced form) once their live pieces ship, same treatment as `05`/`08` — full narrative content of all four files captured in `improvement-142` first. Pieces 1/2/4/5/7 are concretely specified (exact files/functions/line numbers) and independently runnable; pieces 3/6 still have open decisions (rule priority, unverified blast radius) |
