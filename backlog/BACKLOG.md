...
|---|---|---|---|
| Top | 🔴 | 201 | improvement-201 — Five bundled findings (per explicit user direction, filed 2026-09-22). **Part 1 (done):** Attachment upload content-type is never validated server-side, a confirmed stored-XSS-via-upload vector — fixed via `AttachmentContentTypeValidator` (Tika magic-byte detection) + shared whitelist. **Part 2 (done):** City/Taxon vertical, 4 class pairs duplicated near-verbatim (~700 lines removed) — extracted into `AbstractTaxonManagementView`/`AbstractTaxonOverlay`/`AbstractTaxonViewOverlayModeHandler`/`AbstractTaxonFormOverlayModeHandler`, extending the established `AbstractEntityOverlay<H>`/`LocaleTranslationForm<T>` pattern; `Taxon*`-named Category-specific classes renamed to `Category*`. **Part 3/4 (unverified):** `UserService.cleanup()` orchestrator-relocation candidate; generic exception crossing a module boundary in `ProviderProfileSaveService`. **Part 5 (unverified):** `AbstractTaxonOverlay.proceed()` silent no-op on empty post-save refetch, pre-existing, found during Part 2. Placed above every other backlog item |
