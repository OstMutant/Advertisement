...
|---|---|---|---|
| Top | 🟡 | 152 | improvement-152 (filed 2026-08-13, split out of improvement-151 once that issue's own real work was ready to close). Part A: `unit-tests.sh` never installs to `~/.m2`, so `integration-tests.sh`'s own staleness check (ADR-007) always finds every module stale right after `unit-tests.sh` just compiled them, and redundantly reinstalls. Fix: a new `scripts/build.sh` (`mvn install -DskipTests` once for the shared reactor modules), called first by `run-all-tests.sh`. Part B: background/decision material for unblocking `improvement-138`'s Track B (ArchUnit-based SPI Map data, gated by `improvement-135` item 5) — not started, investigation only. Part C: SPI Interface Details table redesign (Callers/Implemented By split, Module → Class → Method) — design only, not started |
