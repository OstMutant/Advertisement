...
|---|---|---|---|
| Top | 🔴 | 201 | improvement-201 — Four bundled findings (per explicit user direction, filed 2026-09-22). **Part 1 (done):** Attachment upload content-type is never validated server-side, a confirmed stored-XSS-via-upload vector — fixed via `AttachmentContentTypeValidator` (Tika magic-byte detection) + shared whitelist. **Part 2 (in progress, step 2/4):** City/Taxon vertical, 4 class pairs duplicated near-verbatim (~500+ lines) — the largest confirmed DRY debt in the codebase, being extracted into `AbstractTaxonManagementView`/`AbstractTaxonOverlay` + analogous abstractions, mirroring the established `AbstractEntityOverlay<H>`/`LocaleTranslationForm<T>` pattern. **Part 3/4 (unverified):** `UserService.cleanup()` orchestrator-relocation candidate; generic exception crossing a module boundary in `ProviderProfileSaveService` — both reported via an external review pass, not yet independently confirmed. Placed above every other backlog item |
