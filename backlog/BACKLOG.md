...
|---|---|---|---|
| Top | 🔴 | 147 | improvement-147 (filed 2026-08-07, continuation of `improvement-136`, moved to the very top per explicit user request) — is `orchestrator.shared`'s single-caller-today collaborator shape (`TaxonAssignmentWriteService`/`AttachmentSnapshotReaderService`/`AttachmentSoftDeleteService`, each called only by `AdvertisementSaveService` right now) actually justified? Investigate why no second caller exists yet, track `improvement-124` Batch 124-C's `ProviderProfileSaveService` (now planned to reuse `TaxonAssignmentWriteService`) as the real second-consumer test, then decide keep-as-shared vs. fold-back-to-private once that lands |
...
### Top priority — improvement-147

| Issue | Origin | What |
|---|---|---|
| [improvement-147](issues/improvement-147-marketplace-orchestrator-followups.md) | New (filed 2026-08-07, split out of `improvement-136`'s original 3-item deferred follow-up list at explicit user direction — item 1 merged into `improvement-124` Batch 124-C's own plan since that's where the real work happens, item 2 dropped entirely as not actionable with no REST caller in this codebase) | Three `marketplace-orchestrator` collaborator classes (`TaxonAssignmentWriteService`, `AttachmentSnapshotReaderService`, `AttachmentSoftDeleteService`) were built as shared/reusable but have exactly one real caller each today (`AdvertisementSaveService`). Investigate why, track `improvement-124` Batch 124-C's `ProviderProfileSaveService` (now planned to reuse `TaxonAssignmentWriteService`) as the real second-consumer test once it lands, then decide keep-as-shared vs. fold back into private methods |
