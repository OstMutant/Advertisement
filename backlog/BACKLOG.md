...
|---|---|---|---|
| Top | 🟡 | 174 | improvement-174 (filed 2026-08-27) — Bounded Contexts' `ports_json` (`bounded_contexts_json()`) uses the same regex-based SPI extraction `improvement-156` just replaced for the SPI Map — three spots (UI/Orchestrator/default-starter domains) still text-match `implements`/`ComponentFactory<X>` instead of reading real `spiEdges` bytecode data. Not yet confirmed as a live false positive, but the same class of bug, same fix shape, same already-available data source |
