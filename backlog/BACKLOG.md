...
|---|---|---|---|
| **Top** | 🔴 | 144 | improvement-144 (filed 2026-08-06, **highest priority per explicit user request — first to execute**) — investigate + implement a dedicated top-level "Code Quality" card on the System screen showing every module's SonarQube/ArchUnit metrics side by side (Part A), plus an on-demand refresh trigger from inside the tool (Part B — real design fork to resolve first: a copy-command clipboard button that preserves the tool's current serverless/`file://`-only property, vs. a small local companion server for a true one-click experience at the cost of that property; recommendation with tradeoffs needed before implementing, not a silent default). Part A reuses whatever rendering lands from improvement-142's sibling follow-up below — suggested execution order is 142's follow-up first, then this issue, even though this issue is ranked higher in priority tier |
