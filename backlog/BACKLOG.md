...
|---|---|---|---|
| **Top** | 🔴 | 136 | improvement-136 (new, filed 2026-08-04, highest priority per explicit user request) — enrichment orchestration (`AdvertisementEnrichmentService`/`ProviderProfileEnrichmentService`) lives in the wrong module (domain starters, not marketplace-app); found during Batch 124-B's post-implementation review. Land before improvement-124 Batch 124-C so the `AccountOverlay` UI isn't built against a port contract that's about to change |
...
| Issue | Origin | What |
|---|---|---|
| [improvement-136](issues/improvement-136-enrichment-orchestration-belongs-in-marketplace-app.md) | New (found during improvement-124 Batch 124-B's post-implementation review, 2026-08-04) | `AdvertisementEnrichmentService`/`ProviderProfileEnrichmentService` orchestrate calls across 3 sibling starters' ports (`TaxonPort`/`UserPort`/`AttachmentPort`) from inside their own domain starter, building a `Locale`-resolved display DTO before returning it through the port — contradicts each starter's own "what it owns" boundary and the root `CLAUDE.md`'s explicit assignment of cross-starter stitching to marketplace-app. Evidence: the `Locale` parameter on `AdvertisementPort`/`ProviderProfilePort`'s methods, and the fact that `AdvertisementAuditEnrichService` already does the identical kind of resolution correctly in marketplace-app for audit-diff rendering — the list/card path just never got the same treatment. No existing ADR decided this placement. Fix: drop `Locale` from both ports, delete both `*EnrichmentService` classes from their starters, add per-domain enrichment services in marketplace-app (`services/advertisement/`, new `services/providerprofile/`), mirroring `AdvertisementAuditEnrichService`'s existing shape — not one shared god-service. Detailed plan in the issue file |
