...
|---|---|---|---|
| Top | 🔴 | 176 | improvement-176 (filed 2026-08-28, `/autopilot` review, ranked at the top per explicit user request) — two plain bugs in `.claude/commands/autopilot.md` (duplicate step "4." numbering; a stale/false claim that no git-commit hook exists, contradicting `.claude/rules.md`'s own documented `PreToolUse` hook) plus two findings cross-checked against the private certification document: a real destructive-action safety gap (force-push/`reset --hard`/etc. are prompt-only enforced, unlike the already-hooked git-commit case, in exactly the unattended-run scenario where that's weakest) and a re-surfaced Plan Mode vs. hand-rolled Approval Rule duplication (`improvement-160`'s `D3-6`, now concretely observed in autopilot's own step 1) |
| Top | 🟡 | 175 | improvement-175 (filed 2026-08-28, carved out of `improvement-124` Batch 124-B2 per explicit user request, ranked at the top) — shared `HtmlSanitizerUtil` (`platform-commons`) replacing near-duplicated sanitizer logic in `AdvertisementService`/`ProviderProfileService`, plus a stale-id-during-concurrent-delete fix in both services' `save()`/`buildEntity()` (a real race: row deleted between read and write silently falls back to insert-shaped defaults with a stale id instead of a clear conflict error). Both findings survived `/code-review` verification during Batch 124-B. Gates `improvement-124` Batch 124-C |
...
### Top priority — improvement-176

| Issue | Origin | What |
|---|---|---|
| [improvement-176](issues/improvement-176-autopilot-review-certification-findings.md) | New (user-requested `/autopilot` review, filed 2026-08-28, ranked at the top per explicit user request) | Two plain bugs found by direct inspection of `.claude/commands/autopilot.md`: duplicate step "4." numbering, and a stale/false claim ("this repo has no actual git commit hook") that directly contradicts `.claude/rules.md`'s own documented `PreToolUse:Bash` commit-blocking hook — confirmed live in this same session. Two more findings from cross-checking against `/app/private/AICertificationAndAudit/AICertification.txt` (real quotes, not assumed): (1) autopilot's destructive-action safety net (force-push, `reset --hard`, schema changes beyond scope) is prompt-only enforced, unlike the already-hooked git-commit case — exactly the certification's own prompt-vs-hook reliability argument, applied in exactly the highest-risk (unattended-run) scenario for it; (2) autopilot step 1's "plan, then stop and wait" reimplements in prose what Claude Code's built-in Plan Mode already provides, re-surfacing `improvement-160`'s closed `D3-6` idea with a concrete example |

### Top priority — improvement-175

| Issue | Origin | What |
|---|---|---|
| [improvement-175](issues/improvement-175-shared-sanitizer-stale-id-delete-race.md) | New (carved out of `improvement-124` Batch 124-B2, filed 2026-08-28 per explicit user request) | Two findings from Batch 124-B's `/code-review --fix` pass that survived verification but touch `advertisement-spring-boot-starter` files outside that batch's own scope: (1) `ProviderProfileService`'s HTML-sanitizer/visible-text-length logic near-verbatim duplicates `AdvertisementService`'s — extract a shared `HtmlSanitizerUtil` in `platform-commons`, same precedent as `YoutubeUtil`; (2) a real stale-id-during-concurrent-delete race in both services' `save()`/`buildEntity()` — a row deleted between read and write silently falls through to insert-shaped defaults with a stale id instead of a clear conflict error, fixed identically in both domains so they don't drift into two different error shapes for the same race. Gates `improvement-124` Batch 124-C |
