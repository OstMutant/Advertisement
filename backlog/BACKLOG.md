...
|---|---|---|---|
| **Top** | 🟡 | 135, 124 | improvement-135 — items 1, 2, 4 done (ADR-index + flows.md freshness gates; review-skill token cost tracked as a standing practice, no default effort level changed without evidence; flows.md routing accuracy spot-check 6/6). Item 3 (context-loading.md validation) remains, needs scoping. Then resume F-04/improvement-124 Batches B-D — new `provider-profile-spring-boot-starter` module (`EntityType.PROVIDER_PROFILE`, `ProviderKind` MASTER/SHOP/SUPPORT), plus the unified "My Account" overlay (name + settings + provider profile in one place, narrower moderator edit permissions, reusing improvement-128's `EntityActivityOverlay`) — its own snapshot DTO must follow improvement-002's `schemaVersion` pattern |
...
| Issue | Origin | What |
|---|---|---|
| [improvement-135](issues/improvement-135-ai-nav-layer-validation-and-adr-index-ci-check.md) | New (process/AI-tooling meta, filed 2026-07-31, found while reviewing improvement-124's diff) | Validates improvement-134's `docs/ai/` layer instead of assuming it works: (1) **done** — `docs/ai/adr-index.md` can no longer silently drift from its source `DECISIONS.md` files, via a standing `.claude/rules.md` rule + a read-only `scripts/ai/check-adr-index-freshness.sh` gate in `scripts/ci.sh` (no git hook — confirmed none exists in this repo), plus the index's same-number-different-file ADR collision risk fixed (`ADR-NNN (module)`); item 1's own audit also found `docs/ai/flows.md` missing 6 built-in skills, fixed by splitting it into a mechanically-checked "Project commands" table (`scripts/ai/check-flows-completeness.sh`, same CI gate) and an explicitly-not-checkable "Built-in skills" table; (2) **done** — reframed from the original vague "docs/ai read-count proxy" to the concrete cost question the user actually raised (review-skill token cost, ~1.7M tokens across Batch 124-A/A2's two `/code-review` passes, real numbers not estimates) — adopted as a standing `.claude/rules.md` reporting practice (sum `subagent_tokens` per purpose in every final report), explicitly did **not** lower any review's default effort level on cost alone, since there's no counterfactual evidence a cheaper pass would've caught the same real bugs; (3) validate `context-loading.md` empirically — open, needs scoping; (4) **done (bounded)** — 6 realistic task phrasings sent to isolated subagents blind to `flows.md`, checked against its recommendations: 6/6 correct, including the two closest-confusable pairs (`simplify` vs `/code-review`, `review` vs `/code-review`) — caveat: shows the underlying skill descriptions disambiguate correctly, not proven to isolate `flows.md`'s own incremental value; (5) governing rule — no new `docs/ai/*` content until 3 shows the existing layer earns its cost |
