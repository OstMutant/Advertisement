...
|---|---|---|---|
| Top | 🔴 | 149 | improvement-149 (filed 2026-08-08, explicit user request to rank at the very top) — clarify (or fix) why `System › Diagrams › Module Dependencies — Dependency Graph` didn't change after `improvement-147`'s true-BFF migration while `Bounded Contexts — Context Map` did; explanation given in chat wasn't accepted as clear. Point 1: re-explain concretely, with a live walkthrough of the generator code and rendered diagram, why Module Dependencies looks the way it does. Point 2: nail down concretely what actually differs between the two diagrams (node/edge sets, evidence source, a real litmus-test example that moves one but not the other) |
| Top | 🔴 | 150 | improvement-150 (filed 2026-08-11, explicit user request to rank right after 149) — tighten improvement-149 Point 5's result: `marketplace-app` should depend on nothing but `marketplace-orchestrator`, not even `platform-commons`/`query-lib` as Point 5 currently allows. `query-lib` looks like a free removal (zero real imports found); `platform-commons` is genuinely used by 73 files (DTOs, core model enums, 3 Hook implementations, `AccessEvaluator`'s `UserAuthorizationPort`) and needs a real design, not a mechanical move |
...
| Nice to have | — | 073 → 035, 096, 129, 036, 039, 065, 114, 063, 028, 130, 131, 133, 148 | everything else — no internal priority order, pick up opportunistically |
...
### Top priority — improvement-149

| Issue | Origin | What |
|---|---|---|
| [improvement-149](issues/improvement-149-architecture-map-module-deps-vs-bounded-contexts.md) | New (filed 2026-08-08, found while reviewing `improvement-147`'s result — explicit user request to rank at the top) | Two points to resolve: (1) explain why `Module Dependencies — Dependency Graph` didn't change after the true-BFF migration, in a way that actually lands (previous chat explanation rejected); (2) nail down concretely what `Bounded Contexts — Context Map` vs `Module Dependencies` actually differ in, with a real testable litmus-test example — `scripts/architecture/DECISIONS.md` ADR-015 already recorded a decision on this, check it first |
...
| [improvement-148](issues/improvement-148-reverify-optional-module-removal-after-bff-migration.md) | New (filed 2026-08-08, follow-up surfaced while planning `improvement-147`'s true-BFF migration) | Re-run `improvement-136` Phase 8's optional-module-removal proof (strip `taxon`/`provider-profile` starter, rebuild, boot, exercise a dependent feature) once `improvement-147` lands — the migration moves who holds every `ComponentFactory<XPort>`, so the earlier proof needs repeating, not just assuming the mechanism is unchanged. Blocked on `improvement-147` |
