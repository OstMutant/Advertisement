...
`backlog/completed/`).
**Module:** `platform-commons` (new `advertisement.model.ListingType`, DTO fields),
...
1. **`platform-commons`:** new enum `ListingType { OFFER, REQUEST, PRODUCT }` in
   ...
   package exists yet). Add `listingType` to `AdvertisementInfoDto` (`ListingType`, required —
   unlike `cityTaxonId` this is never null after backfill), `AdvertisementSaveDto` (`@NotNull
   ListingType listingType` — new required record component, positioned after `description`
   before `categoryIds` to match creation-form field order), `AdvertisementFilterDto`
   (`Set<ListingType> listingTypes` — plural, multi-select like `categoryIds`/`UserFilterDto
   .roles`, not singular like `cityTaxonId`), and `AdvertisementSnapshotDto` (`ListingType
   listingType` — add to `diff()`/`allFields()` following the exact `cityTaxonId` pattern already
   there, i.e. `Fields.listingType` compared via `Objects.equals` and formatted via
   `listingType.name()` or `""` when null pre-backfill).
2. **Liquibase:** edit `advertisement-spring-boot-starter/src/main/resources/db/advertisement
   ...
   `listing_type VARCHAR(20) NOT NULL DEFAULT 'OFFER'` to the `advertisement` table.
3. **`Advertisement` entity** (`advertisement-spring-boot-starter/.../entity/Advertisement.java`):
   add `ListingType listingType;` field — Spring Data JDBC maps enums to `VARCHAR` natively via
...
4. **`AdvertisementRepository`:**
   - `ROW_MAPPER`: add `.listingType(ListingType.valueOf(rs.getString("listing_type")))`.
   - Add `a.listing_type` to the column list of all three custom `SELECT`s
     (`findAdvertisementById`, `findByCreator`, `findByFilter`).
   - `FILTER`: add `SqlBoundFilter.of(listingTypes, "a.listing_type", (m, v) ->
     inSet(m, v.getListingTypes()))` — this is the literal `UserRepository.roles` pattern
     (`inSet(m, v.getRoles())` on an enum `Set`), the closest existing precedent, confirmed by
     reading `UserRepository.java` directly.
   - `buildEntity()` in `AdvertisementService`: add `.listingType(dto.listingType())`.
5. **`AdvertisementSaveService`:** both `AdvertisementSnapshotDto` construction sites (`after` in
   `save()`, and `buildCurrentSnapshot()`) need `saved.getListingType()`/`ad.getListingType()`
   threaded in as the new positional argument — no taxon lookup needed, `listingType` comes
...
6. **`marketplace-app` UI, mirroring the city/category touchpoints file-for-file:**
   - `AdvertisementEditDto`: add `ListingType listingType` field.
   - `AdvertisementFormOverlayModeHandler`: add a `RadioButtonGroup<ListingType>` field (new
   ...
   - `AdvertisementFilterMeta`: add `LISTING_TYPES` — `FilterFieldMeta<Set<ListingType>,
     AdvertisementFilterDto, Set<ListingType>>`, mirroring `UserFilterMeta.ROLES` exactly (closest
     existing precedent — plain enum `Set`, no DTO-to-id mapping needed unlike `CATEGORY_IDS`).
   - `AdvertisementQueryBlock`: add a "Listing type" filter row — `MultiSelectComboBox<ListingType>`
     with `.setItems(ListingType.values())`, `.setItemLabelGenerator(t ->
     i18nService.get(I18nKey.forListingType(t)))`, mirroring the existing Categories row's shape
   ...
   - `AdvertisementCardView`: two additions per the spec's explicit split — (1) a small type badge
     (new `Span`, positioned near the title, using `I18nKey.forListingType(...)`, mirroring the
   ...
   - `AdvertisementActivityFieldsHookImpl.labelFor()`: **add the `listingType` case from the
     ...
     .listingType -> i18n.get(I18nKey.CHANGES_FIELD_LISTING_TYPE);` in the same commit that adds
     the field, not as a follow-up fix.
   - `I18nKey`: typed entries per the "no dynamic keys" rule already in force — one key per type
     for the radio label/filter item/badge text (`ADVERTISEMENT_LISTING_TYPE_OFFER`/`_REQUEST`/
     `_PRODUCT`), plus a `CHANGES_FIELD_LISTING_TYPE` for the activity-diff label, plus a static
     `I18nKey.forListingType(ListingType)` switch method mirroring the existing
...
7. **Testing — extend existing tests, not new spec files** (explicit project preference from the
   ...
   `advertisement.flow.js` — add a `listingType` param mirroring the `city` param added for F-02),
   ...
   `listingType` field, mirroring the `cityTaxonId` diff tests added in improvement-119's
...
- [improvement-119](../completed/issues/improvement-119-f02-city-dictionary-geo-filter.md) — F-02,
  ...
  AND-branch for `listingTypes` — verify during implementation whether `listingTypes` should
