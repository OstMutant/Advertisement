...
| D2-2 | Real MCP server integration — SonarQube | done (2026-08-26) — `improvement-173`. **Correction to the original 2026-08-19 finding:** the npm package `sonarqube-mcp-server` is deprecated/no longer supported (verified directly against its real GitHub README); the actual current official distribution is the Docker image `sonarsource/sonarqube-mcp`. Also **not** a session-wide `.mcp.json` as originally proposed — implemented as an agent-scoped inline `mcpServers` entry (`.claude/agents/sonar/sonar-analyst.md`), which reconnects fresh on every dispatch instead of once per Claude Code session, since a session-wide server can't be individually restarted mid-session if its SonarQube auth token goes stale. `SONARQUBE_TOOLSETS=quality-gates,issues,measures` scopes it to 5 tools (per Domain 2's own 4-5-tools-per-agent guidance, D2-1 below). Verified live end to end — real quality-gate/metrics data retrieved via `Agent({subagent_type: "sonar-analyst"})` | done |
| D2-3 | Real MCP server integration — Dagu | done (2026-08-26) — `improvement-173`. **Correction to the original finding:** not proxied at `localhost:8082` — that proxy exists only for a real browser to reach the Dagu UI; `ci-runner` runs `--network host` (same as `claude-dev`), so `localhost:18080/mcp` is directly reachable container-to-container. Implemented as an agent-scoped HTTP-type `mcpServers` entry (`.claude/agents/dagu/dagu-analyst.md`), read-only by default. Real, unfixable-by-us limitation found: MCP connections are established before an agent's own instructions run, so the agent starting `ci-runner` itself mid-dispatch doesn't make the MCP tools available within that same dispatch — it falls back to Dagu's own REST API directly in that case, confirmed returning real (non-fabricated) run data. The `dagu_read`/`dagu_change`/`dagu_execute` MCP tools themselves remain unverified live end to end (would need `ci-runner` already running before a dispatch); the REST fallback path is what's actually confirmed working | done |
| D2-4 | MCP server scoping — project vs user config | done (2026-08-26) — `improvement-173`, but **not the mechanism originally recommended here.** The project-vs-user distinction itself held (whatever's checked into git and shared with the team is right, a personal `~/.claude.json` is wrong) — but a root `.mcp.json` specifically turned out wrong for both SonarQube and Dagu: SonarQube's auth token is too volatile for a session-wide server with no way to reconnect it individually, and Dagu's server may not be running yet when the session starts. Both ended up as agent-scoped inline `mcpServers` entries (`.claude/agents/sonar/sonar-analyst.md`, `.claude/agents/dagu/dagu-analyst.md`) instead — still project-scoped (checked into git, shared with the team) but connecting fresh per-dispatch rather than once per session. No root `.mcp.json` exists in this repo | done |
...
| D3-3 | Command/skill frontmatter `argument-hint`/`allowed-tools` | Exact YAML syntax confirmed 2026-08-19 via the real frontmatter reference table at [code.claude.com/docs/en/skills](https://code.claude.com/docs/en/skills): `allowed-tools` accepts a space- or comma-separated string or a YAML list; `argument-hint` is valid for project skills (the "unexpected key" error only applies when packaging for the external Agent Skills spec). done — `infra-doc-standards`/`module-doc-standards` (verified 2026-08-19: neither had any frontmatter at all, not just these two fields; `doc-standards` was these two skills' pre-split name, no longer exists as its own skill) now have `name`/`description`/`allowed-tools`. No `argument-hint` added to either — confirmed neither uses `$ARGUMENTS`, so the field would be moot. **Re-verified 2026-08-26:** `deep-review/SKILL.md` no longer exists (deleted entirely per `improvement-171`, replaced by the `.claude/agents/*.md`-based `deep-review-orchestrator`); no `argument-hint`/`$ARGUMENTS`-using skill file has one. All 12 real `.claude/commands/*.md` files still lack it — core finding still holds, citation updated | done (partial) |
...
| D3-7 | Situation → which command/skill handles it (workflow routing) | **Correction (2026-08-20): not sourced from the private certification document** — checked directly (`grep`/`sed` against `/app/private/AICertificationAndAudit/AICertification.txt`); the document's only "Situation → X" table (Domain 3) maps a situation to a prompt-refinement *technique* (examples vs. iteration vs. interview pattern), not to a command/skill. `.claude/nav/flows.md` — a real, working routing table mapping "situation" to the exact command/skill that handles it and why, kept fresh by `.claude/nav/scripts/check-flows-completeness.sh` wired into `scripts/ci.sh`'s `docs` stage — is a genuine, useful repo mechanism, just not a certification-sourced point | not certification |
| D3-8 | Session Context Isolation — independent review instances counter same-session self-justification bias | Verbatim from the real document, "CI/CD Integration" task statement, "Session Context Isolation" subsection: "The same Claude session that generated code is less effective at reviewing its own changes... The fix: independent review instances — use a separate Claude Code invocation for review — one that has no access to the generation session's reasoning context." done — `.claude/skills/deep-review`'s verify-agent step (a fresh subagent confirms every finding) and `infra-readme-standards/SKILL.md`'s own "Independent review" section (not `infra-doc-standards` as originally cited here) are the two existing repo mechanisms that already match this shape. See the "Practical evidence log" below for a live, observed occurrence of the failure this fixes. **Re-verified 2026-08-26:** `.claude/skills/deep-review` no longer exists (deleted per `improvement-171`) — its successor, the `deep-review-orchestrator` agent, dispatches a fresh, separate verification subagent per finding via `Agent`, an equivalent (and more structurally enforced) instance of the same Session Context Isolation shape. `infra-readme-standards/SKILL.md`'s "Independent review" section still exists unchanged. Concept still holds | done |
...
| D5-5 | Task type → what context to load, instead of loading speculatively | **Correction (2026-08-20): not sourced from the private certification document** — checked directly; no passage describes a "task type → which files to open" routing table. The closest real concept (path-scoped `.claude/rules/` loading only for matching file paths, Domain 3) is a different mechanism — automatic load-by-path, not a manual task-type reading guide. `.claude/nav/context-loading.md` is a real, working repo mechanism, just not a certification-sourced point | not certification |
| D5-6 | Avoid duplicating full source content into a summary/index — generate, don't hand-author | **Correction (2026-08-20): not sourced from the private certification document** — checked directly, zero matches for "duplicat"/"hand-author"/"summary/index" language describing this concept. `.claude/nav/adr-index.md` — mechanically generated one-line-per-ADR index (never restates decision text) so a session doesn't have to open every full `DECISIONS.md` file speculatively; regenerated automatically on every `DECISIONS.md` edit per a standing `.claude/rules.md` rule — is a real repo mechanism, just not a certification-sourced point | not certification |
| D5-7 | Information provenance / drift detection (a claim's source must stay checkably fresh) | **Certification concept — verbatim from the real document, Domain 5:** "Information provenance — knowing where every claim comes from and how confident you should be in it — is the difference between a research system that produces trustworthy outputs and one that produces plausible-sounding fiction." The document's own context is claim attribution across a multi-agent research synthesis pipeline — a related but distinct application of the same idea to this repo's own doc-freshness problem. done — `.claude/nav/scripts/check-adr-index-freshness.sh` (documented in `.claude/nav/README.md`'s "Staying correct" section) is a read-only freshness check that diffs the live `.claude/nav/adr-index.md` against a fresh regeneration, wired as an unconditional early stage in `scripts/ci.sh`, catching drift between `DECISIONS.md` and its generated index automatically. See the "Practical evidence log" below. **Re-verified 2026-08-26:** file and wiring (`scripts/ci/dagu/ci.yaml`'s `docs` stage) both confirmed still real | done |
...
### `.claude/nav/scripts/check-adr-index-freshness.sh` (existing repo mechanism, not new work this session)
...
**How we address it:** `.claude/nav/adr-index.md` is a generated index of every `DECISIONS.md` entry
...
- **Where:** `.claude/nav/scripts/check-adr-index-freshness.sh`, `.claude/nav/README.md`'s "Staying correct"
  section, `scripts/ci.sh`'s `docs` stage.
- **How:** regenerate the index in a scratch location, diff it against the committed one; any
  difference means `.claude/nav/adr-index.md` is stale relative to the real `DECISIONS.md` files.
