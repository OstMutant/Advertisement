...
(`GET /api/qualitygates/project_status`, `GET /api/tasks/search`), not assumed:
- Quality gate: `OK` — all 3 conditions green, `new_violations` actual value `0`.
- Project-wide: `GET /api/tasks/search?componentKeys=advertisement&resolved=false` returns
