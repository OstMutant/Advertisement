...
- `session_manifest.md` (a durable scratch of "active rules"/"discussed constraints") has real
  ...
  write the current plan into the relevant `backlog/tasks/<n>.md` file before presenting it.
...
- `backlog/tasks/improvement-167-dag-aware-agent-friendly-script-execution-contract.md` — same
...
| `project_run_all_tests_orchestrator.md` | STALE | cites dead `unit-tests.sh`/`integration-tests.sh` names and a `features/tasks/` path that doesn't match the current `backlog/tasks/` convention |
...
`backlog/tasks/improvement-133-deferred-oversized-review-findings.md` instead of being dropped
silently.
