...
**Status:** done. P2/P3 (deferred at the time) both shipped later via
...
## Deferred at the time — both since shipped via improvement-006

### P2 — QuillEditor character counter — ✅ done

The limit was already decided and enforced: `AdvertisementSaveDto.DESCRIPTION_MAX_LENGTH = 2000`,
checked in the binder validator in `AdvertisementFormOverlayModeHandler.buildBinder()`. What was
still missing at the time was UI feedback — a character counter reading raw text length from
Quill. The known tag-spam vulnerability in the length-checking regex (see
[issue-description-length-tag-spam](issue-description-length-tag-spam.md)) was fixed first
(Jsoup-based text extraction), then the counter itself shipped together with the DB limit change
via [improvement-006](improvement-006-quill-description-counter-and-db-limit.md).

### P3 — `advertisement.description` DB schema — ✅ done

Shipped via the same [improvement-006](improvement-006-quill-description-counter-and-db-limit.md) —
widened to `VARCHAR(20000)` (raw-HTML cap, not the 2000 visible-text limit — see
`marketplace-app/DECISIONS.md` ADR-031), not the smaller value originally suggested here.
...
| `marketplace-app/.../fields/QuillEditor.java` | character counter — done (improvement-006) |
