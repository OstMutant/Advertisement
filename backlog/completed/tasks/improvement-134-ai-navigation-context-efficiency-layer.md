...
(`backlog/tasks/improvement-134-ai-navigation-context-efficiency-layer.md`), updated in place with
...
1. **Audit** — delegate to a research subagent (Explore or general-purpose, "very thorough"
   ...
   - Read every module `CLAUDE.md`, every `DECISIONS.md`, `docs/architecture/03-bounded-contexts.md`
     ...
     `BACKLOG-ARCHIVE.md` + `backlog/tasks/` + `backlog/completed/tasks/`,
...
(`backlog/completed/tasks/improvement-120-advertisement-user-hard-fk-coupling.md`) and its
...
(`backlog/tasks/` → a name that doesn't imply bugs-only), mechanical `git mv` + grep-replace across
...
which `DECISIONS.md` file(s)/backlog tasks/architecture doc(s) are worth opening, explicitly
...
3. `docs/architecture/06-coupling-analysis.md:239` lists `improvement-011` as `~ PARTIAL`/open —
   not re-verified against `backlog/completed/tasks/` in this pass, `UNKNOWN`.
...
- `backlog/tasks/improvement-129-marketplace-feed-modernization.md` — precedent for a large
  proposal stored as its own issue file.
- `backlog/tasks/improvement-131-priority-emoji-rubric-doc-practice-mismatch.md` — closest
  existing precedent for a structured metadata-on-markdown convention; must reconcile with, not
  duplicate.
- `backlog/tasks/improvement-133-deferred-oversized-review-findings.md` — overlap check target.
- `backlog/tasks/improvement-130-backlog-issues-folder-rename.md` — overlap check target (backlog
...
- `improvement-120` (completed, `backlog/completed/tasks/improvement-120-advertisement-user-hard-fk-coupling.md`,
...
- **`/code-review --fix`** (8 parallel finder angles, high effort): found and fixed a real
  ...
  `docs/ai/README.md` cross-reference (pointed at `backlog/completed/tasks/` before the issue was
