...
`DECISIONS.md` files and individual `backlog/tasks/*.md`/`backlog/completed/tasks/*.md` files are
...
`backlog/completed/tasks/*.md` file (which this project already archives permanently and never
...
- Does "prune" mean **delete the superseded ADR's text entirely** from `DECISIONS.md` (relying on
  `backlog/completed/tasks/` and git history for recovery), or **shrink it to a one-line pointer**
  ("ADR-044: superseded by ADR-070, see completed/tasks/improvement-XXX.md for the original text")?
...
- Not every ADR traces back to a `backlog/completed/tasks/*.md` file 1:1 — several were recorded
...
- `backlog/tasks/*.md`/`backlog/completed/tasks/*.md` — unchanged; these remain the permanent
