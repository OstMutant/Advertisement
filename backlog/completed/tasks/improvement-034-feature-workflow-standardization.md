...
## Correction (2026-07-16): `backlog/<name>/SPEC.md` is not what's actually in use — retarget to `backlog/tasks/`
...
`tasks/`/`completed/`. The convention that actually won out in practice (every issue filed this
session, and the existing ones before it) is one markdown file per issue directly in
`backlog/tasks/<prefix>-NNN-<slug>.md`, with a consistent header block (`**Type:**`/`**Module:**`/
...
- A template capturing the shape already in consistent use across `backlog/tasks/*.md`: the
...
- A `/feature <name>` skill (or `/issue <name>`, naming TBD — "feature" undersells that this covers
  bugs/tech-debt too, not just new features) that scaffolds a new `backlog/tasks/<prefix>-NNN-<slug>.md`
...
`backlog/tasks/<prefix>-NNN-<slug>.md` from the shape formalized above (auto-numbered by scanning
both `backlog/tasks/` and `backlog/completed/tasks/` for the highest existing number across all
