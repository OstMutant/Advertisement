...
| D3-7 | Situation → which command/skill handles it (workflow routing) | `docs/ai/flows.md` — a real, working routing table mapping "situation" to the exact command/skill that handles it and why, kept fresh by `docs/ai/scripts/check-flows-completeness.sh` wired into `scripts/ci.sh`'s `docs` stage. Already covered — cite `docs/ai/flows.md` | already covered |
...
| D4-3 | Structured output for machines, separate from the human-readable view | `docs/architecture/architecture-model.json` (machine-readable system model) is generated separately from `docs/architecture/architecture-map.html` (human visual explorer) by the same `generate-architecture-model.sh` script — the same "structured data for a consumer that parses it, prose/visuals for a consumer that reads it" split this domain's tool_use/JSON-schema concept describes, already real and working. Already covered — cite `docs/architecture/architecture-model.json` | already covered |
...
| D5-5 | Task type → what context to load, instead of loading speculatively | `docs/ai/context-loading.md` — a real, working routing table (task type → which `DECISIONS.md`/backlog/architecture files are worth opening) that exists specifically to avoid opening files "speculatively". Already covered — cite `docs/ai/context-loading.md` | already covered |
| D5-6 | Avoid duplicating full source content into a summary/index — generate, don't hand-author | `docs/ai/adr-index.md` — mechanically generated one-line-per-ADR index (never restates decision text) so a session doesn't have to open every full `DECISIONS.md` file speculatively; regenerated automatically on every `DECISIONS.md` edit per a standing `.claude/rules.md` rule. Already covered — cite `docs/ai/adr-index.md` and `docs/ai/scripts/generate-adr-index.sh` | already covered |
| D5-7 | Information provenance / drift detection (a claim's source must stay checkably fresh) | `docs/ai/README.md`'s "Staying correct" section — a read-only freshness check (`docs/ai/scripts/check-adr-index-freshness.sh`) diffs the live index against a fresh regeneration and is wired as an unconditional early stage in `scripts/ci.sh`, catching drift between `DECISIONS.md` and its generated index automatically. Already covered — cite `docs/ai/README.md`'s "Staying correct" section | already covered |
...
**Idea reference:** none — checked every row in Domain 1-5 (`D1-1` through `D5-7`); no existing row
captures "independent review instances counter same-session self-justification bias" as its own
idea. `D1-1` ("Verify-agent discipline") is adjacent but distinct — it proposes formally wiring
verify-agent discipline into `/deep-review` via `ReportFindings`, an unimplemented idea (`status:
idea`), not the same thing as the live event this entry records.
