...
- Precise consumer counts (corrected during a later merge with a pre-existing, more carefully
  counted `features/leaf-widgets-plain-classes/SPEC.md` — the initial `grep` pass in this issue
...
3. **`PaginationBar.java` already uses a plain constructor**, not `Configurable`/`Parameters` —
   ...
   **Reconciled disagreement:** the pre-existing `features/leaf-widgets-plain-classes/SPEC.md`
...
- `features/completed/issues/improvement-011-unguarded-port-injection-in-ui-components.md` /
...
- `features/leaf-widgets-plain-classes/SPEC.md` — a pre-existing, independently-written spec for
