...
| [improvement-045](issues/improvement-045-critical-test-coverage-gaps.md) | 8 critical untested code paths found via targeted audit (AccessEvaluator authorization chokepoint, User `@Version` split, login/register rate-limit asymmetry, 2 taxon soft-delete SQL gaps, translation fallback, user restore, settings diff) | **items 1, 2, 3, 4/5 done (2026-07-14)**: `AccessEvaluatorTest` (17/17) + `AuthServiceTest`/`UserServiceTest` (5/5 each — rate-limit asymmetry locked in) + `UserRepositoryTest` (3/3 — `updateProfile()` optimistic locking + email/passwordHash immutability) + `TaxonRepository.findByIds()`/`findByTypeAndCode()` SQL fix; items 6/7/8 still open (translation fallback, user restore, settings diff) |
