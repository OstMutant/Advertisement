...
| [improvement-045](issues/improvement-045-critical-test-coverage-gaps.md) | 8 critical untested code paths found via targeted audit (AccessEvaluator authorization chokepoint, User `@Version` split, login/register rate-limit asymmetry, 2 taxon soft-delete SQL gaps, translation fallback, user restore, settings diff) | high for items 1/3 (security); **items 4/5 done (2026-07-14)**: caller trace showed both were dead code (unreachable from production, not a live bug as first suspected), fixed `TaxonRepository.findByIds()`/`findByTypeAndCode()` SQL + `TaxonRepositoryTest` (4/4, proven red→green); items 1/2/3/6/7/8 still open |
