...
| [improvement-045](issues/improvement-045-critical-test-coverage-gaps.md) | 8 critical untested code paths found via targeted audit (AccessEvaluator authorization chokepoint, User `@Version` split, login/register rate-limit asymmetry, 2 taxon soft-delete SQL gaps, translation fallback, user restore, settings diff) | **items 1, 2, 3, 4/5, 6, 7 done (2026-07-15)**: `AccessEvaluatorTest` (17/17) + `AuthServiceTest`/`UserServiceTest` (5/5 each) + `UserRepositoryTest` (3/3) + `TaxonRepository` SQL fix + `TaxonPortTranslationFallbackTest` (4/4) + `UserServiceRestoreTest` (2/2 — tested via public `UserService.restoreToSnapshot()`, not the private method, see `integration-tests/DECISIONS.md` ADR-008); item 8 still open (settings diff) |
