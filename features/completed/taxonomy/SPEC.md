...
| UI components for category management — in the starter or in marketplace-app? | **In marketplace-app.** The starter is a pure domain/data module (no Vaadin dependency). All Vaadin components (Reference Data tab, category selector, chips, filter field) are written from scratch in marketplace-app and use `TaxonPort` directly. No `TaxonUiPort` SPI is needed. |
