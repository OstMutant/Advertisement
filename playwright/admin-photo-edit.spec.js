...
const AD_TITLE = 'Admin Photo Edit Bug Test';
const avatar   = seed => `https://api.dicebear.com/9.x/adventurer/png?seed=${seed}&size=256&backgroundColor=b6e3f4`;

test.describe('Admin photo edit — single current-state badge', () => {

  test('editing another user ad photo as admin shows exactly one current-state badge', async ({ page }) => {
    const img1 = '/tmp/admin-photo-1.png';
    const img2 = '/tmp/admin-photo-2.png';
    await downloadPng(avatar('admin-photo-a'), img1);
    await downloadPng(avatar('admin-photo-b'), img2);

    // ── Step 1: user creates ad (text only, no photo) ──────────────────────
    await test.step('User creates ad and edits text', async () => {
      ...
      // Edit text so we get v2 in audit_log (text-only change, no photo)
    ...
    // ── Step 2: admin adds a photo ─────────────────────────────────────────
    await test.step('Admin adds photo (audit v3 + photo_snapshot rn=1)', async () => {
    ...
    // ── Step 3: admin replaces the photo ──────────────────────────────────
    await test.step('Admin replaces photo (audit v4 + photo_snapshot rn=2)', async () => {
      const overlay = page.locator('.advertisement-overlay');
    ...
    await test.step('History shows exactly one current-state badge', async () => {
      ...
      await screenshot(page, 'admin-photo-edit-history');
