...
public class TaxonManagementView extends Div {
    ...
    private void refresh() {
        try {
            ...
            List<TaxonDto> all     = taxonCatalogService.listAllByType(TaxonType.CATEGORY, java.util.Locale.ENGLISH, true);
            Map<Long, Long> counts = all.isEmpty() ? Map.of() : taxonCatalogService.getUsageCounts(TaxonType.CATEGORY);
    ...
    private void updateRowInPlace(TaxonDto fresh) {
        ...
        listContainer.getChildren()
                ...
                .ifPresent(old -> {
                    ...
                    long usageCount = taxonCatalogService.getUsageCounts(TaxonType.CATEGORY).getOrDefault(fresh.getId(), 0L);
