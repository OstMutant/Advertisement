...
- `AttachmentPort`, `AttachmentAuditPort` live in `platform-commons` (`AttachmentMediaChangeHook`
  does not exist, see `marketplace-app/DECISIONS.md` ADR-035; `AttachmentAuditPort` — see
  `platform-commons/DECISIONS.md` ADR-025).
...
- `AttachmentPort.getMediaSummaries(EntityType, Set<Long>)` (bulk) computes the "main attachment"
  ...
  column. See `marketplace-app/DECISIONS.md` ADR-035. There is no single-entity
