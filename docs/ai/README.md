...
|---|---|---|---|---|---|
| [adr-index.md](adr-index.md) | One generated line per ADR across every `DECISIONS.md` (module, status, title) | `DECISIONS.md` files are not auto-loaded (unlike module `CLAUDE.md`) and there are more of them, and more entries, than any single session should open speculatively | Complements `DECISIONS.md` itself — never duplicates decision text, only indexes it | Before filing a new ADR or backlog issue, to check whether something was already decided | Regenerated by `bash docs/ai/scripts/generate-adr-index.sh` (mechanical: parses `## ADR-NNN: Title` + `**Status:**`, zero manual fields) — wired as a mandatory step into `/record-decision` and `/sync-docs` (see `docs/ai/adr-index.md`) and into a standing `.claude/rules.md` rule covering any `DECISIONS.md` edit regardless of workflow, plus a `scripts/ci.sh` freshness gate (`docs/ai/scripts/check-adr-index-freshness.sh`) |
...
| [flows.md](flows.md) | Situation → which existing command/skill handles it, and why that one | The operational surface (`/feature`, `/record-decision`, `/sync-docs`, `/code-review`, `deep-review`, `/autopilot`, `/ci`, ...) was scattered across `CLAUDE.md` and a dozen `.claude/commands/*.md` files with no single map | Points at `.claude/commands/*.md`/`.claude/skills/*` — never restates their internal steps | Whenever unsure which command/skill applies to the task at hand | Split in two: its "Project commands & skills" table is checked by `bash docs/ai/scripts/check-flows-completeness.sh` (wired into `scripts/ci.sh`'s `docs` stage + a standing `.claude/rules.md` rule); its "Built-in Claude Code skills" table can't be mechanically checked from this repo (those aren't repo files) — re-verified only during `/sync-docs --full-audit` |
...
`docs/architecture/scripts/` script, kept there since it reuses that script's own markdown parser.
...
edit, by any workflow, regenerates the index (`bash docs/ai/scripts/generate-adr-index.sh`) in the same
operation, not only when going through `/record-decision` — and `bash docs/ai/scripts/check-adr-index-freshness.sh`,
