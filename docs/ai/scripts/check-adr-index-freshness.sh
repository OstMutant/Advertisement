...
cp "$COMMITTED" "$BACKUP"
bash "$REPO_ROOT/docs/ai/scripts/generate-adr-index.sh" > /dev/null
...
if ! diff -q "$BACKUP" "$COMMITTED" > /dev/null 2>&1; then
  ...
  echo "Run: bash docs/ai/scripts/generate-adr-index.sh, review the diff, and commit the result."
  exit 1
fi
