...
- A reachable Docker daemon (Testcontainers starts a real ephemeral Postgres container per `mvn
  test` reactor run — the singleton-container pattern, see `.claude/nav/adr-index.md`)
...
as the working directory (see `.claude/nav/adr-index.md`). On a normal developer machine, no env
...
| `taxon/TaxonPortTranslationFallbackTest` | Testcontainers + `@SpringBootTest` | `TaxonPort.findById()`'s translation-fallback chain (requested locale → configured default → first available → blank), tested through the public port, not the package-private `resolveTranslation()` — see `.claude/nav/adr-index.md` |
