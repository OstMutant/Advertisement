...
#   they're dropped silently instead of triggering a model call.
...
# Uses: sed only -- deliberately not jq. AGENTIC_SUCCESS_BLOCK/ERROR_BLOCK's JSON shape is fixed
#   and produced entirely by this repo's own scripts/utils/agentic-output.sh (flat, known field
#   names, no nesting) -- confirmed directly that jq isn't installed in this sandbox and isn't a
#   dependency of any other script in this repo either, so a full JSON parser would be a new,
#   unjustified dependency for a shape this narrow; targeted sed field-extraction is safe here
#   specifically because the producer is our own code, not arbitrary external JSON.
...
#   line; silently ignores every other line in the batch.
